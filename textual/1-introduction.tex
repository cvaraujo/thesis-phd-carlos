...
Insecticide application instructions are generally based on ideal topography
...
Given this hybrid structure, a new problem is introduced and explored in this
thesis, titled the \gls{cbrp}.
...
Figure~\ref{fig:ex1} illustrates an example of an input graph (city map) and
...
attended blocks are represented by the arcs connected to the double square with
...
Addressing the \gls{cbrp} and its extensions requires a combination of
complementary methodological approaches, each targeting a different facet of the
problem. Deterministic optimization, formulated through \gls{milp} models,
provides exact or near-optimal solutions by exploiting the combinatorial
structure of the routing problem, enabling rigorous performance guarantees and
tight bounds on the solution quality. However, deterministic models assume
perfect knowledge of the input data, which rarely holds in practice for Dengue
control, where the progression and spatial distribution of cases are inherently
uncertain. Stochastic optimization overcomes this limitation by explicitly
modeling uncertainty through scenario-based formulations, allowing the
decision-maker to hedge against multiple plausible future outcomes and produce
solutions that are robust across a range of conditions. To generate realistic
scenarios, a \gls{mabs} is particularly well suited, as it captures the
spatial-temporal dynamics of Dengue spread at the city-block level by modeling
the interactions among mosquitoes, humans, and breeding sites, something that
aggregate analytical models cannot reproduce with the same granularity. Finally,
a simheuristic framework bridges simulation and optimization by coupling a
multi-start heuristic with the \gls{mabs}, enabling the exploration of a large
solution space while evaluating candidates under simulated stochastic conditions.
This integration allows decision-makers to obtain high-quality solutions that
account for uncertainty without the computational burden of solving large-scale
stochastic programs to optimality.

\paragraph{Contributions.} This thesis develops a comprehensive set of
methodologies centered on the \gls{cbrp} to optimize insecticide-spraying
routes for Dengue control. The main contributions are summarized as follows.
First, the \gls{cbrp} is formally defined as a new combinatorial optimization
problem, and two families of \gls{milp} formulations are proposed: a path-based
formulation and a walk-based formulation, each with its particularities.
Preprocessing techniques that reduce the graph size without losing optimality are
introduced. To strengthen the bounds obtained from these formulations, four
\gls{lr} variants are derived, which decompose the problem into shortest-path
and knapsack subproblems. On the heuristic side, a greedy constructive algorithm
that combines binary search with knapsack-based block selection and a
block-connection routing heuristic is designed.
Second, a \gls{mabs} grounded in
compartmental epidemiological theory (SIR/SEI) is developed to simulate the
spatial-temporal progression of Dengue cases at the city-block level. The
simulator is calibrated and validated against historical notification data from
the cities of Alto Santo and Limoeiro do Norte, in Cear\'{a}, Brazil.
Third, the \gls{cbrp} is extended to its stochastic counterpart, the \gls{scbrp},
modeled as a two-stage stochastic program in which the first stage selects
blocks to spray under current conditions and the second stage accounts for
uncertain future scenarios. For the \gls{scbrp}, path-based and walk-based
\gls{milp} formulations analogous to the deterministic case are proposed,
together with tailored heuristics that include a local search with multiple
neighborhood structures, a \gls{sa} metaheuristic, and a \gls{sh}
framework that couples a multi-start heuristic with the \gls{mabs} to handle
uncertainty through simulation-generated scenarios. 
Fourth, two comprehensive benchmarks are constructed: a deterministic benchmark with 39~instances derived from real Dengue notification data spanning multiple years in Alto Santo and Limoeiro, and a stochastic benchmark with 36~instances, each comprising
50~scenarios generated by the \gls{mabs}. Extensive computational experiments
demonstrate the effectiveness of the proposed methodologies across both
benchmarks, providing valuable insights to public health authorities and
policymakers working on vector-borne disease control. Moreover, the generality
of the \gls{cbrp} framework makes it a promising approach for optimizing vehicle
routing strategies in other city-block service applications.

\paragraph{Text organization.} The remainder of this thesis is organized as
follows. Chapter~\ref{chp:preliminary-concepts} introduces the foundational
concepts underlying the methodologies developed in this work, including integer
linear programming, \gls{lr}, stochastic optimization, multi-agent-based
simulation, and simheuristics. Chapter~\ref{chp:literature-review} reviews the
related literature on optimization and decision support approaches for Dengue
vector control and on Dengue spread modeling and simulation.
Chapter~\ref{chp:historical-notifications-data-analysis-validation} describes
the historical notification data from Alto Santo and Limoeiro do Norte, the
instance generation procedure, and the statistical methods used to validate the
computational results. Chapter~\ref{chp:dengue-virus-spread-simulation}
presents the proposed \gls{mabs} for simulating Dengue virus spread, including
agent design, computational experiments, and model validation.
Chapter~\ref{chp:deterministic-methodologies-and-results} details the
deterministic \gls{cbrp} formulations, \gls{lrs}, heuristics,
and the corresponding computational experiments.
Chapter~\ref{chp:uncertainty-methodologies-and-results} addresses the
stochastic setting, presenting the \gls{scbrp} formulations, heuristics, the
Simulated Annealing metaheuristic, and the simheuristic framework. Finally,
Chapter~\ref{chap:conclusions} summarizes the findings and provides concluding
remarks.