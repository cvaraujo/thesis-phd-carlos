...
The first stage focuses on deterministic block selection.
...
\begin{algorithm}[H]
	...
	\tcp{Binary Search for Time Allocation}
...
The SolveScenario subroutine implements a binary search strategy to find the optimal
...
\begin{algorithm}[H]
	...
	\If{key exists in cache}{
		$connection\_cost \leftarrow$ cached routing cost\;
	}\Else{
		$connection\_cost \leftarrow$ \Call{HeuristicBlockConnection}{$G$, $SP$, $y$, $key$}\;
	}
...
\begin{algorithm}[H]
	\SetAlgoLined
	\KwData{Graph $G$, shortest path calculator $SP$, selected blocks $blocks$, solution key $key$}
	\KwResult{Routing cost for visiting all blocks}

	\tcp{Special case - single block}
	\If{$|blocks| = 1$}{
		Cache cost as 0\;
		\Return{$0$}\;
	}
...
The space complexity is $O(B \cdot MT + S \cdot B)$ for storing the DP tables and solution vectors.
