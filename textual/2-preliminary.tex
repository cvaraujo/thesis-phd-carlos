...
This chapter introduces the fundamental concepts necessary for understanding
this thesis. Basic concepts from Combinatorial Optimization are assumed to be
known. For a review, the reader is referred to the textbook by Nemhauser and
...
A \gls{lp} problem consists of maximizing (or minimizing) a linear objective function subject to a set of linear constraints. A \gls{lp} can be formulated as
\begin{equation}
	\text{(PLP)}:\quad \max \{ c x : A x \leq b, \; x \in \mathbb{R}^n \},
\end{equation}
...
\begin{equation}
	\text{(PILP)}:\quad \max \{ c x : A x \leq b, \; x \in \mathbb{Z}^n \}.
\end{equation}
...
When only a subset of the decision variables is restricted to integer values while the remaining variables are allowed to be continuous, the resulting problem is known as a \gls{milp}. Formally, a \gls{milp} can be expressed as $\max \{ c_1 x_1 + c_2 x_2 : A_1 x_1 + A_2 x_2 \leq b, \; x_1 \in \mathbb{Z}^{n_1}, \; x_2 \in \mathbb{R}^{n_2} \}$, where $x_1$ represents the integer variables and $x_2$ the continuous ones. \gls{milp} formulations arise naturally in many practical applications, including vehicle routing, scheduling, and network design, where binary or integer variables model discrete decisions (e.g., selecting routes or assigning tasks) while continuous variables capture quantities such as flows or times. The solution methods described below for \gls{ilp}s, including branch-and-bound and branch-and-cut, extend directly to the \gls{milp} setting.

By relaxing the integrality constraints of PILP, one obtains the linear relaxation of the \gls{ilp}. Let $Q \subset \mathbb{R}^n$ denote the feasible region satisfying $A x \leq b$, and let $P = Q \cap \mathbb{Z}^n$ denote the set of feasible integer solutions. The polyhedron $Q$ is said to be a formulation (or model) of the \gls{ilp}. Among the infinitely many possible formulations satisfying $P = Q \cap \mathbb{Z}^n$, one of particular theoretical interest is the convex hull of $P$, denoted $\text{CH}(P)$, that is, the smallest convex polyhedron containing all integer feasible points.
...
Lagrange multiplier for that constraint. The following problem is now defined:
determine the set of multipliers that provide the best (i.e., greatest) dual
bound $z(u)$. To find these values, it is necessary to solve the Lagrangian dual problem
...
to increase $g(x)$. From Proposition~\ref{proposition:convex}, it follows that if
...
Therefore, it is necessary to find a vector $s$ that satisfies
...
$g$ does not increase. Before finding such a vector $s$, the notions of
subgradient and subdifferential are defined.
...
current point, that is, the current point is optimal. The results below allow
the application of this theory to the Lagrangian relaxation technique for \gls{ilp}.
...
Based on the previous definitions and results, a procedure to minimize a
concave function for which a subgradient is known at all points in its domain
can now be presented. For this purpose, a high-level pseudocode of the
subgradient method is presented in Algorithm~\ref{code:subgradient}.
...
represented by random variables. A decision must be made at the present time;
this is called the first-stage (or first-phase) decision, and at a later time,
after its implementation, the uncertainties are revealed. Then, based on the
first-stage decision and the revealed uncertain data, a corrective (recourse)
decision can be made, called the second-stage (or second-phase)
decision~\cite{birge:2011}.

The uncertain data of the problem are represented by a random vector~$\xi$. Of
particular interest is the case where~$\xi$ is a discrete random variable, that
is, it can take values from a finite set $\Xi = \{\xi_{1}, \ldots, \xi_{k}\}$.
Each possible random event is denoted by $\omega \in \Omega$. Alternatively, $\xi$
...
random vector~$y$, of dimension~$n_{2}$. The notation $y(\omega)$ represents the
...
a single random vector, the following is defined:
\[
...
of matrix $T(\omega)$. Of particular interest is the class of \emph{fixed recourse
...
decision~$x$, the following is defined:
\[
...
\end{align}
which is the optimization problem for a given scenario $\xi(\omega)$. It is assumed
...
That is, for each scenario $\xi(\omega)$, problem~\eqref{eq:scenario_problem}
is solved to optimality and the expected value of the objective over all $\omega \in
	\Omega$ is computed. The \gls{rp}, defined by~\eqref{eq:rp} or~\eqref{eq:rp_det}, can be
written as:
...
A practical difficulty of the wait-and-see approach is that a subproblem must be
solved for each scenario~$\xi(\omega)$. A simpler approach is to replace the
...
recourse problem, since only a single future scenario is solved. The
...
of the solution in terms of~\eqref{eq:rp}. The expected value of the \gls{evp}
...
main idea is to represent the states of individuals~\citep{brauer-2008}. In the
Dengue spread simulation proposed in this thesis, the following states are
adopted: susceptible (S),
...
\gls{sa} is a probabilistic metaheuristic that allows the search process to
escape local optima~\cite{kirkpatrick:1983}. The method is a metaphor for a
thermal process known as annealing, used in metallurgy to obtain low-energy
states in a solid. In the physical process, a solid in a thermal bath is
initially heated by increasing the temperature, so that thermal stresses are
released and all particles are randomly distributed in a liquid phase. This is
followed by slow cooling to ambient temperature, so that all particles arrange
themselves in a low-energy state in which solidification occurs.

More specifically, each iteration of \gls{sa} seeks movements from the current
trial solution to an immediate neighbor in the local
neighborhood~\cite{hillier:2013}. The technique starts the search from an
arbitrary initial solution, and for each generated neighbor a calculation is
performed, based on the current temperature, that determines whether the
neighboring solution will replace the current one. At the end of each iteration,
the temperature is reduced. A pseudocode of \gls{sa} is presented in
Algorithm~\ref{alg:sa-preliminary}.

\begin{algorithm}[!ht]
	\caption{\label{alg:sa-preliminary} Simulated Annealing}
	\SetAlgoLined
	\KwResult{Best solution found}
	$s \leftarrow$ initial random solution\;
	$T \leftarrow T_{\max}$\;
	\While{stopping criterion not satisfied}{
		$s' \leftarrow$ neighbor of $s$\;
		$\Delta \leftarrow f(s') - f(s)$\;
		$u \leftarrow$ random value in $[0, 1]$\;
		\If{$\Delta < 0$ \textbf{or} $e^{-\Delta / T} > u$}{
			$s \leftarrow s'$\;
		}
		Reduce $T$\;
	}
...
One of the most important parameters of \gls{sa} is the maximum temperature
$T_{\max}$, since it directly influences the acceptance probability: the higher
the temperature, the greater the probability that a neighboring solution is
accepted, even if it has a worse objective value than the current solution. The
maximum temperature can also be used as a stopping criterion, since at each
iteration the temperature is reduced; alternatively, a maximum execution time
or a minimum temperature threshold can serve as stopping conditions.

The most distinctive characteristic of \gls{sa} lies in the acceptance
probability of worse neighboring solutions. By accepting solutions with inferior
objective values with probability $e^{-\Delta / T}$, the algorithm can escape
what are considered local optima and explore new regions of the solution space.
Common cooling strategies include geometric cooling ($T_{k+1} = \alpha \cdot
T_k$ with $0 < \alpha < 1$) and linear cooling. As $T$ decreases, the
probability of accepting worse solutions diminishes, and the algorithm
increasingly behaves like a deterministic hill-climbing method. Under certain
theoretical conditions on the cooling schedule, \gls{sa} is guaranteed to
converge to the global optimum~\cite{kirkpatrick:1983}.
...
obtained. According to initial estimates, each of these solutions shows a
high performance in a scenario under uncertainty. Now, in order to increase the
accuracy of these estimates, a more intensive simulation is executed on each of
