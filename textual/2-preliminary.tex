...
% Considering the basic concepts from
% Stochastic Programming, we recommend the textbook by Birge and
% Louveaux~\cite{birge:2011}. 
...
Figure~\ref{fig:walk-feasible-solution-example} presents a feasible solution for
the Walk-CBRP, which also is a solution that is not feasible for the Path-CBRP.
...
\begin{figure}[h!]
	\begin{minipage}[c]{.4\textwidth}
		\begin{tikzpicture}[
			...
			\node[state, above of = d, yshift=0.5cm] (l) {$12$};
			\node[state, above of = f, yshift=0.5cm] (m) {$13$};
	...
	\begin{minipage}[c]{.4\textwidth}
		\begin{tikzpicture}[
			...
			\node[state, below of = b, yshift=-0.5cm] (i) {$9$};
			\node[state, rectangle, double, below of = e, yshift=-0.5cm] (j) {$10$};
			\node[state, below of = i, left of = j, xshift=0.4cm] (k) {$11$};
			
			\node[state, above of = d, yshift=0.5cm] (l) {$12$};
			\node[state, rectangle, double, above of = f, yshift=0.5cm] (m) {$13$};
			\node[state, above of = l, left of = m, xshift=0.4cm] (n) {$14$};
			...
			\path[->, draw = black, opacity = 1.0, bend right=10] (b) edge node {} (m);
			\path[->, draw = black, opacity = 1.0, bend right=10] (m) edge node {} (b);
			\path[->, draw = black, opacity = 1.0, bend right=10] (b) edge node {} (e);
			\path[->, draw = black, opacity = 1.0, bend right=10] (e) edge node {} (b);
			\path[->, draw = black, opacity = 1.0, bend right=10] (b) edge node {} (j);
			\path[->, draw = black, opacity = 1.0, bend right=10] (j) edge node {} (b);
...
A traditional two-stage stochastic programming problem, also called a
...
\begin{figure}
	\begin{tikzpicture}
		...
		\tikzstyle{io} = [ellipse, minimum width=2cm, text width=2cm, minimum height=1cm, text centered, draw=black, fill=gray!30!white]
		
		\tikzstyle{process} = [tape, minimum width=1cm, text width=2cm, minimum height=1cm, text centered, draw=black, fill=green!30!white]
		
		\tikzstyle{decision} = [diamond, text width=1cm, aspect=2, text height=1, text centered, draw=black, fill=yellow!30!white]
		
		\tikzstyle{stage} = [rectangle, minimum width=1cm, text width=1cm, minimum height=1cm, draw=black]
		
		\tikzstyle{artificial} = [rectangle, draw=white, font=\bf]
		
		\tikzstyle{arrow} = [thick,->,>=stealth]
		
		% Create states
		\node (start) [startstop] {Stochastic\\Optimization\\Problem};
		
		\node (deterministic) [startstop, right of=start, xshift=9cm] {Deterministic\\Optimization\\Problem};
		...
		\node (promissing-sol) [decision, xshift=-2cm, left of=ml-component] {Promissing\\Solution?};
		\node (surrogate-sol) [decision, xshift=-2cm, left of=fast-sim-component] {Surrogate\\Model\\Available?};
		...
		\node (available-time) [decision, xshift=-2cm, left of=update-sol-list] {Available\\Computation\\Time?};
		...
		% 
		\draw [arrow] (available-time.west) |- node[anchor=east, yshift=-9.5cm] {yes} (metaheuristic);
		
		% Feedback
		\node (art1) [artificial, right of=metaheuristic, xshift=0.5cm] {};
		
		\draw [arrow,dashed,-] (metaheuristic) -- (art1.center);
		\draw [arrow,dashed] (art1.center) |- node[text width=1cm, fill=white, align=center, rotate=270, xshift=-2cm] {Feedback} (ml-component.north east);
		
		% 
		\node (art2) [artificial, right of=ml-component, xshift=0.5cm] {};
		
		\draw [arrow,dashed,-] (ml-component.east) -- (art2.center);
		\draw [arrow,dashed] (art2.center) |- node[text width=1cm, fill=white, align=center, rotate=270, xshift=-2cm] {Feedback} (surrogate-component);
		...
		\node (art4) [artificial, left of=stage1, rotate=90, yshift=6.5cm] {Stage 1};
		...
		\node (art5) [artificial, left of=elite-sol, below of=available-time, xshift=-2.5cm] {};
		\node (art6) [artificial, right of=intensive-sim-component, below of=intensive-sim-component, yshift=1.25cm, xshift=4.5cm] {};
		
		\node (stage2) [stage, fit=(art5) (art6)] {};
		\node (art7) [artificial, left of=stage2, rotate=90, yshift=6.5cm] {Stage 2};
		...
		\node (art8) [artificial, left of=risk-component, below of=elite-sol, xshift=-2.5cm, yshift=0.35cm] {};
		\node (art9) [artificial, right of=risk-component, below of=risk-component, yshift=1.25cm, xshift=4.5cm] {};
		
		\node (stage3) [stage, fit=(art8) (art9)] {};
		\node (art10) [artificial, left of=stage3, rotate=90, yshift=6.5cm] {Stage 3};
