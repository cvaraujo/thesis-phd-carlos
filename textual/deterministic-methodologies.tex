...
This chapter presents the deterministic methodologies developed in this thesis and the results obtained.
...
block $b$ has an associated set of arcs that can be
serviced by a vehicle.

A formal definition of the \gls{cbrp} is now presented. Let a Planar Graph,
extracted from a real city map, be represented as a weighted directed graph $G$.
...
\begin{itemize}
	...
	\item $t_b : B \rightarrow \mathbb{N}$ is the service time for block $b \in B$;
	\item $\delta^{+}_{i}$ is the set of arcs with destination $i$;
	\item $\delta^{-}_{i}$ is the set of arcs with source $i$;
	\item For a subset $S \subseteq V$: $A(S) = \{(i,j) \in A : i \in S, j \in S\}$ is the set of arcs within $S$, $V(S)$ is the set of nodes in $S$, and $\delta^{+}(S) = \{(i,j) \in A : i \in S, j \notin S\}$ is the set of outgoing arcs from $S$;
\end{itemize}
...
serve a subset of blocks within a given time limit $T$. This thesis considers two
...
To facilitate modeling, the graph $G$ is augmented by introducing a dummy depot
$0$, from which the route originates, and a set of arcs $\{(0, i), (i, 0) :
	\forall i \in V\}$ with times $t_{i,0} = t_{0,i} = 0$, $\forall i \in V$. Thus,
the modified graph is defined as $G' = (V' = V \cup \{0\}, A' = A \cup \{(0, i),
	(i, 0) : \forall i \in V\})$.

Key properties of the \gls{cbrp} are now presented. First, due to the operational
...
\begin{figure}[h!]
	...
	\begin{minipage}[c]{.31\textwidth}
		...
		\end{tikzpicture}
		\subcaption{Explicit Block Servicing.}
	\end{minipage}
	\hfill
	\begin{minipage}[c]{.31\textwidth}
		\begin{tikzpicture}[
...
the number of arcs entering a node is equal to the number of arcs leaving it.
...
To implement the \gls{mtz} approach, the following additional
...
An alternative approach to enforcing subcycle elimination is to apply a separation heuristic based on max-flow/min-cut computations during the branch-and-bound process. Unlike the lazy constraint approach, which only checks integer solutions, this heuristic is also applied to fractional solutions obtained from the LP relaxation at each node of the branch-and-bound tree.

Given a fractional solution $(\bar{x}, \bar{y})$, the arc capacities in the support graph are set to the relaxed LP solution values $\bar{x}_{i,j}$. For each pair of nodes $(s, t)$ in the solution, a max-flow/min-cut computation is performed. The resulting min-cut identifies a subset $S \subset V$ such that the total capacity of arcs leaving $S$ is minimized. If the sum of arc capacities crossing the identified min-cut is strictly less than the flow required by the $y_{ib}$ variables on either side of the cut, a violated subtour elimination constraint is detected and added to the model.

The implementation uses the preflow push-relabel min-cut algorithm from the Lemon Graph Library~\citep{lemon:2011}, which provides an efficient computation of minimum cuts in directed graphs. This separation procedure strengthens the LP relaxation by adding cutting planes that tighten the fractional feasible region, potentially reducing the number of branch-and-bound nodes required to prove optimality.
...
solution, and achieves values always greater than or equal to the Path-CBRP.
...
The objective~\eqref{eq:walk-of} maximizes the profit collected.
Constraints~\eqref{eq:walk-s-t-all} ensure that the route starts and ends at the
depot. Constraints~\eqref{eq:walk-flow-conservation} ensure the equality of in-degree and out-degree
at each used node. Constraints~\eqref{eq:walk-in-path} ensure the connectivity
to at least one node of any attended block. Constraints~\eqref{eq:walk-max-time}
ensure that the route and the block service time do not exceed the time limit.
Constraints~\eqref{eq:walk-subtour-elimination} ensure that the route does not
contain disconnected subtours. Constraints~\eqref{eq:walk-dom-x}
and~\eqref{eq:walk-dom-y} define the domain of the variables.

For every pair of arcs $a' = (k, l)$ and $a'' = (p, q)$, the family
...
This section presents an adaptation of the original planar graph that allows
computing implicit walks using the formulation presented in
Section~\ref{sec:path-based-formulation}. The approach consists of constructing
a new graph in which blocks are transformed into nodes.
Let:
...
\begin{proof}
	...
	prefix and/or suffix of $S^{*}$ whose nodes do not belong to $V_B$,
	another solution is obtained that preserves the total collected prize and does
...
This adaptation in the original graph allows the use of any \gls{cbrp} formulation
presented in Section~\ref{sec:path-based-formulation} in $G_B$ to generate an
...
This section presents and proves that certain nodes and arcs can be removed from the graph without affecting the optimal solution of the \gls{cbrp}. These rules are useful to reduce the size of the instances and to speed up the resolution of the models. The focus is to take advantage of the characteristics of the problem, such as the grouping of the nodes in the blocks. The main idea is to remove from $G$ vertices and arcs that do not affect the optimal solution. The proposed procedure is based on the removal strategies presented by
\cite{carrabs2021biased} for the \gls{sop}.

For the arc removal procedure, let $B_g$ be a block in the set of street blocks $B$.
...
\begin{proof}
	...
	By replacing the segment $\{u, w, v\}$ in $T^*$ with $\{u, w', v\}$, a new tour $T'$ is obtained, which remains feasible and satisfies $c(T') < c(T^*)$ and $p(T') = p(T^*)$,
...
\begin{align}
	...
	(T - (\sum_{(i,j) \in A} x_{i,j}t_{i,j} + \sum_{b \in B} y_b t_b))                                                                                                & \label{eq:lrsp-of}   \\
	\nonumber                                                                                                                                                     + \sum_{b \in B} \mult{\ref{eq:walk-in-path}}{b}
	(\sum_{i \in V(b)} \sum_{j \in \delta^{-}(i)}x_{i,j} - y_b)                                                                                                   &                    & \\
...
term, the following optimization problems must be solved:
\begin{align}
...
situations, other variations of {\lrsp} are defined by increasing the difficulty
of the subproblem through adding time constraints.
...
other variations of {\lrsp} can be defined by increasing the difficulty of the subproblem.
Thus, the Walk-CBRP formulation is augmented by adding two new redundant constraints:
\begin{align}
...
of time spent on the arcs is limited, the presence of negative costs does not generate
...
practice~\cite{irnich:2006}. The implementations in this thesis use the labeling algorithm
...
The most difficult version of this set of \gls{lrs} is {\lrcspkn}, that uses both
...
\begin{algorithm}[h!]
	...
	\tcp{Initialization}
	$n_B \leftarrow$ number of blocks\;
...
\begin{property}[Optimality of Knapsack Block Selection with Full Time Allocation]
	...
	\begin{proof}
		...
		(here, the number of cases attended) subject to a total time constraint $T$. The greedy heuristic first solves this knapsack problem to select a subset of blocks whose total time does not exceed $T$ and whose total profit is maximized. If, after this selection, there exists at least one possible route to connect the selected blocks within the total time respecting the limit $T$, then there are no additional constraints beyond the knapsack constraint. Therefore, the solution found by the knapsack is not only feasible but also optimal for the \gls{cbrp}, as no other selection of blocks can yield a higher profit without exceeding the time limit. Thus, under the assumption of feasible routing, the knapsack solution is optimal.
	\end{proof}
...
After filling the \gls{dp} table, the algorithm backtracks to reconstruct the actual solution. Starting from the maximum time capacity $T$ and the last block $n$, it checks if including block $i$ contributed to the optimal solution by comparing $dp[i][w]$ with $dp[i-1][w]$. If they differ, block $i-1$ is added to the solution and the remaining capacity is reduced accordingly.
...
Besides the sequence of blocks, the heuristic defines the route that connects those blocks and which node attends to each block. The algorithm uses an approach that varies the sequence of blocks (sorting option) and the distance considered between the blocks (the smallest shortest path between each pair of blocks or the shortest path between a specific node from the current block and a specific node of the next block). The algorithm sorts the blocks to be attended according to the sorting option and the distance criteria for the greedy heuristic, then creates a multi-layered \gls{dag}, described in more detail later in this section, to compute the shortest route that visits all blocks in $\mathcal{B}'$. The shortest route is then computed by finding the shortest path from the artificial depot $s$ to $t$ in the \gls{dag}. The route time is then calculated by summing the time of the arcs in the path and the algorithm maintains the best solution found across all the 8 combinations of $sort\_opt$ and $block\_distance\_opt$.
...
\begin{algorithm}[h!]
	...
	\For{$block\_distance\_opt = 1$ \textbf{to} $2$}{
		\For{$sort\_opt = 0$ \textbf{to} $3$}{
			...
			$path \leftarrow$ \Call{ShortestPathInDAG}{$G_{DAG}(s, t)$}\;
...
\begin{itemize}
	\item When $sort\_opt = 0$, blocks are sorted in ascending order by the cumulative hops of the block nodes. Hops are the number of arcs in the shortest path between a node $i \in V$ and the closest node $j \in V$ of the block $b \in B$ in the Graph $G$. The goal is to prioritize blocks that are, in general, closer to a larger number of other blocks in the graph.
...
This section presents the computational experiments for the deterministic version of the \gls{cbrp}. The experiments are conducted on a set of 39 real-world instances from the \gls{cbrp} benchmark, as reported in Table~\ref{tab:deterministic-benchmark}. The strategies presented in this thesis were implemented in C++ with the Gurobi solver version 12.0.1, maintaining the default settings of its parameters and compiled with GCC version 13.3.0 using the optimization level -O3. The experiments were carried out on an Intel(R) Xeon(R) CPU E5-2630 v4 processor with 10 cores of 2.20 GHz each and 64~GB of RAM. The execution occurred on a 64-bit Ubuntu 18.04 Linux operating system.

A solver execution time limit of 3600 seconds is imposed on each run. Regarding the problem parameters, the vehicle route time limit is set to $T = 120$ minutes, the speed of the vehicle when serving a block ($t_b$) is 10 km/h, and the speed when traveling between blocks ($t_{i, j}$) is 20 km/h. The arc times are converted from meters per second to integer values by multiplying by 10 and taking only the integer part.

Nonparametric analysis, as suggested by~\cite{demsar:2006}, is used to compare the results. The null hypothesis, with a confidence level of $95\%$, is that there is no \gls{ssd} when comparing two or more strategies. An adaptation of the Friedman test, made by Iman and Davenport, is used to compare multiple strategies on a set of different instances, rejecting the null hypothesis when the value of the test is greater than the critical value according to the F-distribution. A {\em post hoc} analysis applying the Nemenyi test is then performed to identify the group of strategies that are statistically equivalent, which occurs when the average ranks of two samples differ by at most a given critical difference. For the comparison of two strategies, the Wilcoxon test is used, rejecting the null hypothesis if the result of the test is smaller than $-1.96$.
...
Notably, the Path-CBRP-Frac achieved only 10 optimal solutions (25.6\%) with an average gap of 21.61\%, approximately six times higher than the other base formulations. The average runtime increased to 2933.01 seconds, and the solutions attended fewer blocks on average (29.9 versus 34.2--34.4). These results suggest that fractional cut generation is counterproductive for this problem, as the computational overhead of generating and managing a large number of cuts outweighs the potential benefits of strengthening the LP relaxation.
...
This section presents the main results of the walk-based methodologies, starting from the mathematical formulations, followed by the Lagrangean Relaxation, the Greedy Heuristic, and a Lower and Upper Bounds Analysis.

\subsubsection{Mathematical Formulations}\label{subsec:walk-based-mathematical-formulation}

Since the use of the preprocessing for the Path-based formulations achieved significant improvements in the solution quality, it is natural to extend this approach to the walk-based formulation. In addition, the use of fractional cuts for the path-based formulation proved to be detrimental to the solution quality and will not be used in the walk-based formulation. Considering that the results achieved with fractional cuts and without preprocessing, for the formulations, were fully dominated by the results with preprocessing and without fractional cuts considering both lower and upper bounds, those results will not be presented. With this, two variants of the walk-based formulation will be evaluated.
...
\begin{itemize}
	...
	\item \textsc{Walk-CBRP-MTZ}: This implementation includes \gls{mtz} constraints for subcycle elimination directly in the model and use the transitive closure of the graph to generate the implicit walk solution.
\end{itemize}

Tables~\ref{tab:results-det-walk-cbrp-prep-exp} and~\ref{tab:results-det-walk-cbrp-mtz} present the results of the Walk-CBRP-Prep and Walk-CBRP-MTZ models, respectively. The results are presented in the same format as the results of the Path-CBRP-Prep and Path-CBRP-MTZ models.

\begin{table}[ht!]
	\centering
	\caption{Results of the Model Walk-CBRP-Prep.} \label{tab:results-det-walk-cbrp-prep-exp}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lrrrrrrrrrr}
			...
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Exec.\\ Time (s)\end{tabular}}}                                                                        \\
			\midrule
			\rowcolor{rowgrey}alto-santo-1000-1                                                    & 98  & 282  & 19  & 19 & 39  & 39  & 0,00  & 579   & 426004  & 205   \\
			\rowcolor{rowlight}alto-santo-1000-2                                                   & 122 & 370  & 45  & 37 & 238 & 241 & 1,24  & 2791  & 665200  & 3.604 \\
			\rowcolor{rowlight}alto-santo-1000-3                                                   & 122 & 370  & 45  & 38 & 246 & 248 & 0,81  & 2849  & 401984  & 3.601 \\
			\rowcolor{rowlight}alto-santo-1000-4                                                   & 122 & 370  & 45  & 38 & 253 & 255 & 0,78  & 2894  & 438492  & 3.601 \\
			\rowcolor{rowlight}alto-santo-1000-5                                                   & 122 & 370  & 45  & 38 & 269 & 271 & 0,74  & 2987  & 606350  & 3.602 \\
			\rowcolor{rowlight}alto-santo-1000-6                                                   & 127 & 386  & 49  & 38 & 319 & 323 & 1,24  & 2702  & 600672  & 3.602 \\
			\rowcolor{rowgrey}alto-santo-2000-1                                                    & 99  & 286  & 19  & 19 & 39  & 39  & 0,00  & 499   & 93964   & 26    \\
			\rowcolor{rowlight}alto-santo-2000-2                                                   & 153 & 450  & 47  & 33 & 246 & 249 & 1,20  & 4285  & 444036  & 3.601 \\
			\rowcolor{rowlight}alto-santo-2000-3                                                   & 153 & 450  & 47  & 32 & 252 & 255 & 1,18  & 5932  & 423398  & 3.601 \\
			\rowcolor{rowlight}alto-santo-2000-4                                                   & 153 & 450  & 47  & 32 & 259 & 262 & 1,15  & 2777  & 704414  & 3.602 \\
			\rowcolor{rowlight}alto-santo-2000-5                                                   & 153 & 450  & 47  & 32 & 276 & 279 & 1,08  & 3241  & 542262  & 3.601 \\
			\rowcolor{rowlight}alto-santo-2000-6                                                   & 155 & 458  & 51  & 35 & 325 & 330 & 1,52  & 2655  & 728294  & 3.603 \\
			\rowcolor{rowlight}alto-santo-3000-1                                                   & 141 & 386  & 20  & 19 & 39  & 40  & 2,50  & 29561 & 162260  & 3.600 \\
			\rowcolor{rowlight}alto-santo-3000-2                                                   & 193 & 558  & 49  & 33 & 247 & 249 & 0,80  & 6189  & 483496  & 3.601 \\
			\rowcolor{rowlight}alto-santo-3000-3                                                   & 193 & 558  & 49  & 33 & 251 & 255 & 1,57  & 2827  & 683856  & 3.602 \\
			\rowcolor{rowlight}alto-santo-3000-4                                                   & 193 & 558  & 49  & 33 & 258 & 265 & 2,64  & 2532  & 1016942 & 3.608 \\
			\rowcolor{rowlight}alto-santo-3000-5                                                   & 193 & 558  & 49  & 33 & 275 & 279 & 1,43  & 2733  & 621038  & 3.602 \\
			\rowcolor{rowlight}alto-santo-3000-6                                                   & 195 & 566  & 53  & 2  & 91  & 330 & 72,42 & 3684  & -       & 3.603 \\
			\midrule
			\rowcolor{rowlight}limoeiro-1000-1                                                     & 285 & 872  & 40  & 5  & 8   & 56  & 85,71 & 2534  & 831204  & 3.603 \\
			\rowcolor{rowlight}limoeiro-1000-2                                                     & 300 & 926  & 49  & 34 & 64  & 69  & 7,25  & 3041  & 360954  & 3.601 \\
			\rowcolor{rowlight}limoeiro-1000-3                                                     & 313 & 988  & 68  & 2  & 23  & 113 & 79,65 & 4368  & 768910  & 3.601 \\
			\rowcolor{rowlight}limoeiro-1000-4                                                     & 313 & 988  & 73  & 4  & 30  & 114 & 73,68 & 4376  & 1345212 & 3.601 \\
			\rowcolor{rowlight}limoeiro-1000-5                                                     & 316 & 1012 & 85  & 5  & 12  & 146 & 91,78 & 2439  & 1301196 & 3.603 \\
			\rowcolor{rowlight}limoeiro-1000-6                                                     & 333 & 1088 & 142 & 4  & 33  & 413 & 92,01 & 5215  & 617206  & 3.601 \\
			\rowcolor{rowlight}limoeiro-1000-7                                                     & 333 & 1088 & 146 & 4  & 35  & 438 & 92,01 & 5852  & 666880  & 3.603 \\
			\rowcolor{rowlight}limoeiro-2000-1                                                     & 658 & 2016 & 79  & 3  & 4   & 72  & 94,44 & 4749  & 1276194 & 3.605 \\
			\rowcolor{rowlight}limoeiro-2000-2                                                     & 717 & 2224 & 110 & 2  & 7   & 107 & 93,46 & 4919  & 1366934 & 3.606 \\
			\rowcolor{rowlight}limoeiro-2000-3                                                     & 728 & 2289 & 142 & 4  & 30  & 155 & 80,65 & 4586  & 1277892 & 3.604 \\
			\rowcolor{rowlight}limoeiro-2000-4                                                     & 729 & 2295 & 149 & 3  & 8   & 159 & 94,97 & 7314  & 768274  & 1.121 \\
			\rowcolor{rowlight}limoeiro-2000-5                                                     & 776 & 2473 & 189 & 3  & 13  & 191 & 93,19 & 6327  & 1213032 & 3.603 \\
			\rowcolor{rowlight}limoeiro-2000-6                                                     & 798 & 2589 & 325 & 6  & 41  & 562 & 92,70 & 10023 & 1624492 & 3.602 \\
			\rowcolor{rowlight}limoeiro-2000-7                                                     & 801 & 2599 & 334 & 16 & 91  & 625 & 85,44 & 3776  & 721932  & 1.402 \\
			\rowcolor{rowlight}limoeiro-3000-1                                                     & 850 & 2592 & 87  & 2  & 8   & 76  & 89,47 & 2900  & 1114170 & 3.611 \\
			\rowcolor{rowlight}limoeiro-3000-2                                                     & 891 & 2756 & 120 & 4  & 14  & 106 & 86,79 & 5812  & 1251740 & 3.602 \\
			\rowcolor{rowlight}limoeiro-3000-3                                                     & 910 & 2837 & 154 & 2  & 24  & 156 & 84,62 & 3256  & 572700  & 533   \\
			\rowcolor{rowlight}limoeiro-3000-4                                                     & 910 & 2839 & 161 & 5  & 30  & 160 & 81,25 & 3885  & 783558  & 1.168 \\
			\rowcolor{rowlight}limoeiro-3000-5                                                     & 929 & 2947 & 202 & 3  & 10  & 192 & 94,79 & 7326  & 981856  & 3.603 \\
			\rowcolor{rowlight}limoeiro-3000-6                                                     & 949 & 3049 & 345 & 11 & 85  & 594 & 85,69 & 5131  & 1442712 & 3.617 \\
			\rowcolor{rowlight}limoeiro-3000-7                                                     & 952 & 3067 & 356 & 17 & 122 & 606 & 79,87 & 8053  & 1140258 & 3.600 \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
\begin{table}[ht!]
	\centering
	\caption{Results of the Model Walk-CBRP-MTZ.}
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lrrrrrrrrrr}
			...
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Exec.\\ Time (s)\end{tabular}}}                                                                     \\
			\midrule
			\rowcolor{rowgrey}alto-santo-1000-1                                                    & 69  & 4692   & 19  & 19 & 39  & 39  & 0,00  & 1   & 0 & 10,29    \\
			\rowcolor{rowlight}alto-santo-1000-2                                                   & 116 & 13340  & 45  & 38 & 237 & 242 & 2,07  & 35  & 0 & 289,68   \\
			\rowcolor{rowlight}alto-santo-1000-3                                                   & 116 & 13340  & 45  & 35 & 240 & 250 & 4,00  & 31  & 0 & 320,10   \\
			\rowcolor{rowlight}alto-santo-1000-4                                                   & 116 & 13340  & 45  & 25 & 217 & 256 & 15,23 & 33  & 0 & 280,29   \\
			\rowcolor{rowlight}alto-santo-1000-5                                                   & 116 & 13340  & 45  & 34 & 257 & 273 & 5,86  & 31  & 0 & 266,28   \\
			\rowcolor{rowlight}alto-santo-1000-6                                                   & 118 & 13806  & 49  & 34 & 310 & 326 & 4,91  & 21  & 0 & 331,00   \\
			\rowcolor{rowgrey}alto-santo-2000-1                                                    & 69  & 4692   & 19  & 19 & 39  & 39  & 0,00  & 1   & 0 & 10,27    \\
			\rowcolor{rowlight}alto-santo-2000-2                                                   & 139 & 19182  & 47  & 29 & 233 & 251 & 7,17  & 16  & 0 & 819,49   \\
			\rowcolor{rowlight}alto-santo-2000-3                                                   & 139 & 19182  & 47  & 25 & 229 & 257 & 10,89 & 15  & 0 & 596,88   \\
			\rowcolor{rowlight}alto-santo-2000-4                                                   & 139 & 19182  & 47  & 33 & 253 & 265 & 4,53  & 18  & 0 & 662,37   \\
			\rowcolor{rowlight}alto-santo-2000-5                                                   & 139 & 19182  & 47  & 23 & 231 & 282 & 18,09 & 15  & 0 & 569,61   \\
			\rowcolor{rowlight}alto-santo-2000-6                                                   & 141 & 19740  & 51  & 28 & 296 & 333 & 11,11 & 15  & 0 & 627,50   \\
			\rowcolor{rowlight}alto-santo-3000-1                                                   & 103 & 10506  & 20  & 18 & 38  & 40  & 5,00  & 156 & 0 & 266,15   \\
			\rowcolor{rowlight}alto-santo-3000-2                                                   & 173 & 29756  & 49  & 21 & 197 & 251 & 21,51 & 6   & 0 & 890,87   \\
			\rowcolor{rowlight}alto-santo-3000-3                                                   & 173 & 29756  & 49  & 25 & 214 & 258 & 17,05 & 5   & 0 & 980,54   \\
			\rowcolor{rowlight}alto-santo-3000-4                                                   & 173 & 29756  & 49  & 30 & 231 & 265 & 12,83 & 5   & 0 & 1.262,39 \\
			\rowcolor{rowlight}alto-santo-3000-5                                                   & 173 & 29756  & 49  & 31 & 272 & 281 & 3,20  & 5   & 0 & 1.006,11 \\
			\rowcolor{rowlight}alto-santo-3000-6                                                   & 175 & 30450  & 53  & 31 & 310 & 333 & 6,91  & 3   & 0 & 949,27   \\
			\midrule
			\rowcolor{rowlight}limoeiro-1000-1                                                     & 155 & 23870  & 40  & 31 & 47  & 57  & 17,54 & 9   & 0 & 956,08   \\
			\rowcolor{rowlight}limoeiro-1000-2                                                     & 184 & 33672  & 49  & 28 & 57  & 71  & 19,72 & 3   & 0 & 1.694,55 \\
			\rowcolor{rowlight}limoeiro-1000-3                                                     & 226 & 50850  & 68  & 3  & 8   & 116 & 93,10 & 1   & 0 & 692,64   \\
			\rowcolor{rowlight}limoeiro-1000-4                                                     & 234 & 54522  & 73  & 2  & 2   & 118 & 98,31 & 1   & 0 & 595,51   \\
			\rowcolor{rowlight}limoeiro-1000-5                                                     & 259 & 66822  & 85  & 3  & 7   & 149 & 95,30 & 1   & 0 & 831,80   \\
			\rowcolor{rowlight}limoeiro-1000-6                                                     & 327 & 106602 & 142 & 5  & 11  & -   & -     & -   & - & 763,13   \\
			\rowcolor{rowlight}limoeiro-1000-7                                                     & 328 & 107256 & 146 & 3  & 13  & -   & -     & -   & - & 775,57   \\
			\rowcolor{rowlight}limoeiro-2000-1                                                     & 325 & 105300 & 79  & 2  & 6   & -   & -     & -   & - & 748,53   \\
			\rowcolor{rowlight}limoeiro-2000-2                                                     & 440 & 193160 & 110 & 3  & 6   & -   & -     & -   & - & 359,39   \\
			\rowcolor{rowlight}limoeiro-2000-3                                                     & 512 & 261632 & 142 & 3  & 6   & -   & -     & -   & - & 121,54   \\
			\rowcolor{rowlight}limoeiro-2000-4                                                     & 527 & 277202 & 149 & 3  & 5   & -   & -     & -   & - & 147,82   \\
			limoeiro-2000-5                                                                        & 776 & 2473   & 189 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-2000-6                                                                        & 798 & 2589   & 325 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-2000-7                                                                        & 801 & 2599   & 334 & -  & -   & -   & -     & -   & - & -        \\
			\rowcolor{rowlight}limoeiro-3000-1                                                     & 430 & 184470 & 87  & 2  & 2   & -   & -     & -   & - & 1.594,31 \\
			limoeiro-3000-2                                                                        & 891 & 2756   & 120 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-3000-3                                                                        & 910 & 2837   & 154 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-3000-4                                                                        & 910 & 2839   & 161 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-3000-5                                                                        & 929 & 2947   & 202 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-3000-6                                                                        & 949 & 3049   & 345 & -  & -   & -   & -     & -   & - & -        \\
			limoeiro-3000-7                                                                        & 952 & 3067   & 356 & -  & -   & -   & -     & -   & - & -        \\
			\bottomrule
		\end{tabular}%
	}
\end{table}

Considering the results presented in Table~\ref{tab:results-det-walk-cbrp-prep-exp}, the Walk-CBRP-Prep formulation achieved two optimal solutions for the Alto Santo instances. These instances represent the smallest problem configurations with 19 blocks each. All other Alto Santo instances showed small gaps ranging from 0.74\% to 2.64\%, except for the instance alto-santo-3000-6 which presented a significantly larger gap of 72.42\%, indicating substantial difficulty in solving this particular configuration, which may be attributed to specific cases distribution that challenge the approach to find feasible solutions. The Limoeiro instances presented considerably larger gaps for the Walk-CBRP-Prep formulation, with values ranging from 7.25\% to 94.79\%. The smaller Limoeiro instances (limoeiro-1000-2) achieved the best results with a gap of 7.25\%, while the majority of instances exhibited gaps above 80\%. This huge contrast with Alto Santo results highlights the increased complexity of the Limoeiro urban structure, which presents a significantly larger road network. The execution times for most instances reached the 1 hour time limit, indicating that the solver struggled to improve solutions or close the optimality gap within the allocated computational resources.

Considering the lazy cuts column, it is possible to observe that the Walk-CBRP-Prep formulation spent significant computational effort detecting and eliminating infeasible solutions through the lazy constraint callback mechanism. For Alto Santo instances that achieved optimal or near-optimal solutions, the number of lazy cuts ranged from 93964 to 1.016.942, with smaller instances generally requiring fewer cuts. In contrast, Limoeiro instances generated substantially more lazy cuts (360.954 to 1.624.492), reflecting the increased difficulty in enforcing subtour elimination constraints on these larger and more complex networks.

Considering the results presented in Table~\ref{tab:results-det-walk-cbrp-mtz}, the Walk-CBRP-MTZ formulation achieved two optimal solutions for the Alto Santo instances, matching the Walk-CBRP-Prep performance on these smallest instances. The remaining Alto Santo instances showed gaps ranging from 2.07\% to 21.51\%, generally larger than those obtained by Walk-CBRP-Prep. For Limoeiro instances, the Walk-CBRP-MTZ formulation encountered significant challenges, with gaps ranging from 17.54\% to 98.31\%. More critically, several instances from Limoeiro failed to find feasible \gls{ub}s within the time limit, as indicated by the dashes in the columns. The failure to obtain any bound is due to the huge memory consumption required to solve the problem, exceeding 50~GB of memory before solving even the root node of the branch-and-bound tree.

The comparison between the two walk-based formulations reveals that Walk-CBRP-Prep consistently outperforms Walk-CBRP-MTZ in terms of solution feasibility and gap quality. While Walk-CBRP-MTZ requires no lazy cuts (as subtour elimination is handled through MTZ constraints), this comes at the cost of a significantly weaker formulation that struggles to find feasible solutions on larger instances. The lazy constraint approach, despite requiring more computational effort to manage the dynamic constraint generation, proves more robust and capable of producing solutions across the entire test set. The execution times for Walk-CBRP-Prep consistently reach or approach the time limit, indicating effective utilization of computational resources, whereas Walk-CBRP-MTZ often terminates prematurely on difficult instances without finding feasible solutions.

\subsubsection{Lagrangean Relaxation}\label{subsec:lagrangean-relaxation}

The Lagrangean Relaxation of the walk-based formulation is obtained by dualizing the max time constraint (\eqref{eq:walk-max-time}) and the constraints that associate the attended blocks to the path (\eqref{eq:walk-in-path}).
Six variations of the LR-RCSP-KN were evaluated, using as base the version LR-RCSP-KN, that uses the Boost algorithm to compute \gls{rcsp} problems and the dynamic programming algorithm to compute \gls{kn} problems, all the lagrange multipliers are initialized with 0 and makes no use of preprocessing or heuristics to improve the solution. The variations include the following suffixes in the name:

\begin{itemize}
	\item \textsc{BM}: This version uses the starting set of lagrange multipliers is the one obtained by the resulting of the variables from the dual solution of the original model by the Barrier Method.
	\item \textsc{HR}: This version use the greedy heuristic presented in Section~\ref{sec:greedy-block-conn-route} to generate the starting lower bound and apply the \gls{kn} into the route generated by solving the \gls{lpp} at each step of the subgradient method to generate a feasible set of attended blocks.
	\item \textsc{Prep}: This version uses the preprocessing steps described in Section~\ref{sec:preprocessing} to improve the solution.
\end{itemize}

Considering the essential parameters used in the subgradient method, one of the stopping criteria is the execution time, limiting the execution to 1800 seconds (30 minutes). The following combinations of values were evaluated:

\begin{itemize}
	\item step size ($\pi$) = $[1.0, 1.5, 2.0]$;
...
The results achieved by the different combinations of parameters do not show a significant difference in the quality of the \gls{ub}. For this reason, Tables~\ref{tab:results-lagrange-relax-default} and~\ref{tab:results-lagrange-relax-preprocessing} report the results of the version with the highest quality of \gls{ub}: $\pi = 1.5$, \textsc{maxIter} = 5000, $updatePi = 50$ and reduction factor of $\pi = 0.95$.

\begin{table}[ht!]
	\centering
	\caption{Results of Lagrangean Relaxations Without Preprocessing.}
	\label{tab:results-lagrange-relax-default}
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lrrr|rrrrrr|rrrrrr|rrrrrr|rrrrrr}
			...
			\multicolumn{1}{c}{}                                                             &
			\multicolumn{1}{c|}{}                                                            &
			\multicolumn{6}{c|}{\textbf{LR-RCSP-KN}}                                         &
			\multicolumn{6}{c|}{\textbf{LR-RCSP-KN-HR}}                                      &
			\multicolumn{6}{c|}{\textbf{LR-RCSP-KN-BM}}                                      &
			\multicolumn{6}{c}{\textbf{LR-RCSP-KN-HR-BM}}                                                                                                                                                                                                          \\
			\cmidrule(lr){5-10}\cmidrule(lr){11-16}\cmidrule(lr){17-22}\cmidrule(lr){23-28}
			\multicolumn{1}{c}{\textbf{Instance}}                                            &
			\multicolumn{1}{c}{\textbf{|V|}}                                                 &
			\multicolumn{1}{c}{\textbf{|A|}}                                                 &
			\multicolumn{1}{c|}{\textbf{|B|}}                                                &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}First\\ LB\end{tabular}}}  &
			\multicolumn{1}{c}{\textbf{LB}}                                                  &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}First\\ UB\end{tabular}}}  &
			\multicolumn{1}{c}{\textbf{UB}}                                                  &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Gap\\ (\%)\end{tabular}}}  &
			\multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Time\\ (s)\end{tabular}}} &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}First\\ LB\end{tabular}}}  &
			\multicolumn{1}{c}{\textbf{LB}}                                                  &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}First\\ UB\end{tabular}}}  &
			\multicolumn{1}{c}{\textbf{UB}}                                                  &
			...
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Time\\ (s)\end{tabular}}}                                                                                                                                                                        \\
			\midrule
			alto-santo-1000-1                                                                & 179  & 524  & 84  & 0 & 13 & 39   & 39  & 66,67 & 29 & 39  & 39  & 39   & 39  & 0,00  & 0  & 0 & 13 & 39   & 39  & 66,67 & 32 & 39  & 39  & 39   & 39  & 0,00  & 0  \\
			alto-santo-1000-2                                                                & 179  & 524  & 84  & 0 & 43 & 246  & 244 & 82,38 & 36 & 238 & 238 & 246  & 244 & 2,46  & 23 & 0 & 43 & 246  & 243 & 82,30 & 33 & 238 & 238 & 246  & 243 & 2,06  & 26 \\
			alto-santo-1000-3                                                                & 179  & 524  & 84  & 0 & 46 & 253  & 251 & 81,67 & 36 & 245 & 245 & 253  & 251 & 2,39  & 26 & 0 & 46 & 253  & 250 & 81,60 & 31 & 245 & 245 & 253  & 250 & 2,00  & 26 \\
			alto-santo-1000-4                                                                & 179  & 524  & 84  & 0 & 50 & 260  & 258 & 80,62 & 27 & 252 & 252 & 260  & 258 & 2,33  & 23 & 0 & 50 & 260  & 257 & 80,54 & 33 & 252 & 252 & 260  & 257 & 1,95  & 29 \\
			alto-santo-1000-5                                                                & 179  & 524  & 84  & 0 & 61 & 277  & 274 & 77,74 & 28 & 268 & 268 & 277  & 274 & 2,19  & 27 & 0 & 61 & 277  & 273 & 77,66 & 29 & 268 & 268 & 277  & 273 & 1,83  & 32 \\
			alto-santo-1000-6                                                                & 179  & 524  & 84  & 0 & 79 & 338  & 330 & 76,06 & 35 & 318 & 318 & 338  & 330 & 3,64  & 23 & 0 & 79 & 338  & 328 & 75,91 & 39 & 318 & 318 & 338  & 328 & 3,05  & 31 \\
			alto-santo-2000-1                                                                & 253  & 710  & 102 & 0 & 13 & 39   & 39  & 66,67 & 49 & 39  & 39  & 39   & 39  & 0,00  & 0  & 0 & 13 & 39   & 39  & 66,67 & 49 & 39  & 39  & 39   & 39  & 0,00  & 0  \\
			alto-santo-2000-2                                                                & 253  & 710  & 102 & 0 & 43 & 268  & 254 & 83,07 & 51 & 246 & 246 & 268  & 254 & 3,15  & 24 & 0 & 43 & 268  & 252 & 82,94 & 32 & 246 & 246 & 268  & 252 & 2,38  & 25 \\
			alto-santo-2000-3                                                                & 253  & 710  & 102 & 0 & 46 & 275  & 260 & 82,31 & 53 & 251 & 251 & 275  & 260 & 3,46  & 26 & 0 & 46 & 275  & 258 & 82,17 & 30 & 251 & 251 & 275  & 258 & 2,71  & 51 \\
			alto-santo-2000-4                                                                & 253  & 710  & 102 & 0 & 50 & 282  & 267 & 81,27 & 29 & 258 & 258 & 282  & 267 & 3,37  & 24 & 0 & 50 & 282  & 265 & 81,13 & 53 & 258 & 258 & 282  & 265 & 2,64  & 51 \\
			alto-santo-2000-5                                                                & 253  & 710  & 102 & 0 & 61 & 300  & 284 & 78,52 & 51 & 275 & 275 & 300  & 284 & 3,17  & 26 & 0 & 61 & 300  & 282 & 78,37 & 54 & 275 & 275 & 300  & 282 & 2,48  & 27 \\
			alto-santo-2000-6                                                                & 253  & 710  & 102 & 0 & 79 & 364  & 337 & 76,56 & 52 & 324 & 324 & 364  & 337 & 3,86  & 32 & 0 & 79 & 364  & 335 & 76,42 & 36 & 324 & 324 & 364  & 335 & 3,28  & 33 \\
			alto-santo-3000-1                                                                & 353  & 964  & 128 & 0 & 13 & 41   & 41  & 68,29 & 26 & 39  & 39  & 41   & 41  & 4,88  & 26 & 0 & 13 & 41   & 40  & 67,50 & 31 & 39  & 39  & 41   & 40  & 2,50  & 30 \\
			alto-santo-3000-2                                                                & 353  & 964  & 128 & 0 & 54 & 296  & 254 & 78,74 & 28 & 246 & 246 & 296  & 254 & 3,15  & 26 & 0 & 49 & 296  & 252 & 80,56 & 35 & 246 & 246 & 296  & 252 & 2,38  & 28 \\
			alto-santo-3000-3                                                                & 353  & 964  & 128 & 0 & 54 & 303  & 260 & 79,23 & 28 & 251 & 251 & 303  & 260 & 3,46  & 25 & 0 & 49 & 303  & 258 & 81,01 & 34 & 251 & 251 & 303  & 258 & 2,71  & 28 \\
			alto-santo-3000-4                                                                & 353  & 964  & 128 & 0 & 57 & 310  & 267 & 78,65 & 28 & 258 & 258 & 310  & 267 & 3,37  & 25 & 0 & 57 & 310  & 265 & 78,49 & 34 & 258 & 258 & 310  & 265 & 2,64  & 28 \\
			alto-santo-3000-5                                                                & 353  & 964  & 128 & 0 & 70 & 330  & 284 & 75,35 & 28 & 275 & 275 & 330  & 284 & 3,17  & 25 & 0 & 61 & 330  & 282 & 78,37 & 33 & 275 & 275 & 330  & 282 & 2,48  & 29 \\
			alto-santo-3000-6                                                                & 353  & 964  & 128 & 0 & 92 & 399  & 337 & 72,70 & 26 & 323 & 323 & 399  & 337 & 4,15  & 25 & 0 & 92 & 399  & 335 & 72,54 & 32 & 323 & 323 & 399  & 335 & 3,58  & 29 \\
			\midrule
			limoeiro-1000-1                                                                  & 400  & 1260 & 231 & 0 & 8  & 57   & 57  & 85,96 & 26 & 51  & 51  & 57   & 57  & 10,53 & 26 & 0 & 8  & 57   & 57  & 85,96 & 28 & 51  & 51  & 57   & 57  & 10,53 & 28 \\
			limoeiro-1000-2                                                                  & 400  & 1260 & 231 & 0 & 13 & 79   & 74  & 82,43 & 27 & 65  & 65  & 79   & 74  & 12,16 & 28 & 0 & 13 & 79   & 72  & 81,94 & 32 & 65  & 65  & 79   & 72  & 9,72  & 32 \\
			limoeiro-1000-3                                                                  & 400  & 1260 & 231 & 0 & 27 & 139  & 118 & 77,12 & 28 & 108 & 108 & 139  & 118 & 8,47  & 29 & 0 & 24 & 139  & 117 & 79,49 & 31 & 108 & 108 & 139  & 117 & 7,69  & 32 \\
			limoeiro-1000-4                                                                  & 400  & 1260 & 231 & 0 & 27 & 145  & 120 & 77,50 & 27 & 110 & 110 & 145  & 120 & 8,33  & 28 & 0 & 24 & 145  & 118 & 79,66 & 33 & 110 & 110 & 145  & 118 & 6,78  & 31 \\
			limoeiro-1000-5                                                                  & 400  & 1260 & 231 & 0 & 28 & 189  & 151 & 81,46 & 30 & 136 & 136 & 189  & 151 & 9,93  & 29 & 0 & 33 & 189  & 149 & 77,85 & 31 & 136 & 136 & 189  & 149 & 8,72  & 30 \\
			limoeiro-1000-6                                                                  & 400  & 1260 & 231 & 0 & 63 & 671  & 441 & 85,71 & 29 & 389 & 389 & 671  & 441 & 11,79 & 29 & 0 & 63 & 671  & 431 & 85,38 & 32 & 389 & 389 & 671  & 431 & 9,74  & 32 \\
			limoeiro-1000-7                                                                  & 400  & 1260 & 231 & 0 & 70 & 719  & 468 & 85,04 & 30 & 415 & 415 & 719  & 468 & 11,32 & 30 & 0 & 70 & 719  & 458 & 84,72 & 33 & 415 & 415 & 719  & 458 & 9,39  & 32 \\
			limoeiro-2000-1                                                                  & 1008 & 3112 & 549 & 0 & 8  & 110  & 77  & 89,61 & 52 & 59  & 59  & 110  & 77  & 23,38 & 50 & 0 & 8  & 110  & 75  & 89,33 & 73 & 59  & 59  & 110  & 75  & 21,33 & 62 \\
			limoeiro-2000-2                                                                  & 1008 & 3112 & 549 & 0 & 16 & 184  & 114 & 85,96 & 54 & 87  & 87  & 184  & 114 & 23,68 & 49 & 0 & 16 & 184  & 111 & 85,59 & 71 & 87  & 87  & 184  & 111 & 21,62 & 65 \\
			limoeiro-2000-3                                                                  & 1008 & 3112 & 549 & 0 & 23 & 274  & 159 & 85,53 & 54 & 129 & 129 & 274  & 159 & 18,87 & 48 & 0 & 27 & 274  & 155 & 82,58 & 69 & 129 & 129 & 274  & 155 & 16,77 & 66 \\
			limoeiro-2000-4                                                                  & 1008 & 3112 & 549 & 0 & 23 & 288  & 163 & 85,89 & 51 & 132 & 132 & 288  & 163 & 19,02 & 52 & 0 & 27 & 288  & 159 & 83,02 & 67 & 132 & 132 & 288  & 159 & 16,98 & 63 \\
			limoeiro-2000-5                                                                  & 1008 & 3112 & 549 & 0 & 28 & 404  & 204 & 86,27 & 55 & 157 & 157 & 404  & 204 & 23,04 & 49 & 0 & 33 & 404  & 199 & 83,42 & 70 & 157 & 157 & 404  & 199 & 21,11 & 65 \\
			limoeiro-2000-6                                                                  & 1008 & 3112 & 549 & 0 & 63 & 1467 & 603 & 89,55 & 56 & 471 & 471 & 1467 & 603 & 21,89 & 54 & 0 & 63 & 1467 & 587 & 89,27 & 64 & 471 & 471 & 1467 & 587 & 19,76 & 68 \\
			limoeiro-2000-7                                                                  & 1008 & 3112 & 549 & 0 & 79 & 1564 & 642 & 87,69 & 52 & 496 & 496 & 1564 & 642 & 22,74 & 54 & 0 & 79 & 1564 & 626 & 87,38 & 66 & 496 & 496 & 1564 & 626 & 20,77 & 66 \\
			limoeiro-3000-1                                                                  & 1240 & 3798 & 660 & 0 & 7  & 121  & 78  & 91,03 & 55 & 58  & 58  & 121  & 78  & 25,64 & 68 & 0 & 8  & 121  & 76  & 89,47 & 89 & 58  & 58  & 121  & 76  & 23,68 & 83 \\
			limoeiro-3000-2                                                                  & 1240 & 3798 & 660 & 0 & 16 & 199  & 114 & 85,96 & 68 & 89  & 89  & 199  & 114 & 21,93 & 57 & 0 & 16 & 199  & 111 & 85,59 & 82 & 89  & 89  & 199  & 111 & 19,82 & 80 \\
			limoeiro-3000-3                                                                  & 1240 & 3798 & 660 & 0 & 27 & 294  & 159 & 83,02 & 59 & 127 & 127 & 294  & 159 & 20,13 & 67 & 0 & 27 & 294  & 156 & 82,69 & 89 & 127 & 127 & 294  & 156 & 18,59 & 85 \\
			limoeiro-3000-4                                                                  & 1240 & 3798 & 660 & 0 & 27 & 312  & 163 & 83,44 & 69 & 130 & 130 & 312  & 163 & 20,25 & 67 & 0 & 27 & 312  & 160 & 83,13 & 73 & 130 & 130 & 312  & 160 & 18,75 & 84 \\
			limoeiro-3000-5                                                                  & 1240 & 3798 & 660 & 0 & 33 & 446  & 205 & 83,90 & 69 & 156 & 156 & 446  & 205 & 23,90 & 67 & 0 & 33 & 446  & 200 & 83,50 & 77 & 156 & 156 & 446  & 200 & 22,00 & 97 \\
			limoeiro-3000-6                                                                  & 1240 & 3798 & 660 & 0 & 63 & 1593 & 611 & 89,69 & 69 & 457 & 457 & 1593 & 611 & 25,20 & 62 & 0 & 63 & 1593 & 595 & 89,41 & 91 & 457 & 457 & 1593 & 595 & 23,19 & 88 \\
			limoeiro-3000-7                                                                  & 1240 & 3798 & 660 & 0 & 79 & 1712 & 652 & 87,88 & 59 & 493 & 493 & 1712 & 652 & 24,39 & 69 & 0 & 79 & 1712 & 635 & 87,56 & 75 & 493 & 493 & 1712 & 635 & 22,36 & 88 \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
\begin{table}[ht!]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{lrrr|rrrrrr|rrrrrr|rrrrrr|rrrrrr}
			...
			\multicolumn{1}{c}{\textbf{LB}}                                                   &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Initial\\ UB\end{tabular}}} &
			\multicolumn{1}{c}{\textbf{UB}}                                                   &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Gap\\ (\%)\end{tabular}}}   &
			\multicolumn{1}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Time\\ (s)\end{tabular}}}  &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Initial\\ LB\end{tabular}}} &
			...
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Time\\ (s)\end{tabular}}}                                                                                                                                                                        \\
			\midrule
			alto-santo-1000-1                                                                 & 100 & 304  & 19  & 0 & 13 & 39   & 39  & 66,67 & 7  & 39  & 39  & 39   & 39  & 0,00  & 0  & 0 & 13 & 39   & 39  & 66,67 & 8  & 39  & 39  & 39   & 39  & 0,00  & 0  \\
			alto-santo-1000-2                                                                 & 127 & 388  & 45  & 0 & 43 & 246  & 244 & 82,38 & 10 & 238 & 238 & 246  & 244 & 2,46  & 9  & 0 & 43 & 246  & 243 & 82,30 & 11 & 238 & 238 & 246  & 243 & 2,06  & 8  \\
			alto-santo-1000-3                                                                 & 127 & 388  & 45  & 0 & 46 & 253  & 251 & 81,67 & 10 & 245 & 245 & 253  & 251 & 2,39  & 9  & 0 & 46 & 253  & 250 & 81,60 & 11 & 245 & 245 & 253  & 250 & 2,00  & 9  \\
			alto-santo-1000-4                                                                 & 127 & 388  & 45  & 0 & 50 & 260  & 258 & 80,62 & 9  & 252 & 252 & 260  & 258 & 2,33  & 9  & 0 & 50 & 260  & 257 & 80,54 & 11 & 252 & 252 & 260  & 257 & 1,95  & 9  \\
			alto-santo-1000-5                                                                 & 127 & 388  & 45  & 0 & 61 & 277  & 274 & 77,74 & 10 & 268 & 268 & 277  & 274 & 2,19  & 9  & 0 & 61 & 277  & 273 & 77,66 & 10 & 268 & 268 & 277  & 273 & 1,83  & 9  \\
			alto-santo-1000-6                                                                 & 130 & 400  & 49  & 0 & 79 & 338  & 330 & 76,06 & 10 & 319 & 319 & 338  & 330 & 3,33  & 9  & 0 & 79 & 338  & 328 & 75,91 & 11 & 319 & 319 & 338  & 328 & 2,74  & 10 \\
			alto-santo-2000-1                                                                 & 100 & 304  & 19  & 0 & 13 & 39   & 39  & 66,67 & 7  & 39  & 39  & 39   & 39  & 0,00  & 0  & 0 & 13 & 39   & 39  & 66,67 & 8  & 39  & 39  & 39   & 39  & 0,00  & 0  \\
			alto-santo-2000-2                                                                 & 159 & 480  & 47  & 0 & 43 & 268  & 254 & 83,07 & 10 & 247 & 247 & 268  & 254 & 2,76  & 8  & 0 & 43 & 268  & 252 & 82,94 & 11 & 247 & 247 & 268  & 252 & 1,98  & 10 \\
			alto-santo-2000-3                                                                 & 159 & 480  & 47  & 0 & 46 & 275  & 260 & 82,31 & 10 & 251 & 251 & 275  & 260 & 3,46  & 9  & 0 & 46 & 275  & 258 & 82,17 & 11 & 251 & 251 & 275  & 258 & 2,71  & 9  \\
			alto-santo-2000-4                                                                 & 159 & 480  & 47  & 0 & 50 & 282  & 267 & 81,27 & 10 & 258 & 258 & 282  & 267 & 3,37  & 9  & 0 & 50 & 282  & 265 & 81,13 & 11 & 258 & 258 & 282  & 265 & 2,64  & 9  \\
			alto-santo-2000-5                                                                 & 159 & 480  & 47  & 0 & 61 & 300  & 284 & 78,52 & 10 & 275 & 275 & 300  & 284 & 3,17  & 9  & 0 & 61 & 300  & 282 & 78,37 & 11 & 275 & 275 & 300  & 282 & 2,48  & 9  \\
			alto-santo-2000-6                                                                 & 159 & 480  & 51  & 0 & 79 & 364  & 337 & 76,56 & 9  & 324 & 324 & 364  & 337 & 3,86  & 9  & 0 & 79 & 364  & 335 & 76,42 & 10 & 324 & 324 & 364  & 335 & 3,28  & 9  \\
			alto-santo-3000-1                                                                 & 142 & 410  & 20  & 0 & 13 & 41   & 41  & 68,29 & 8  & 39  & 39  & 41   & 41  & 4,88  & 8  & 0 & 13 & 41   & 40  & 67,50 & 9  & 39  & 39  & 41   & 40  & 2,50  & 8  \\
			alto-santo-3000-2                                                                 & 195 & 578  & 49  & 0 & 54 & 296  & 254 & 78,74 & 10 & 246 & 246 & 296  & 254 & 3,15  & 9  & 0 & 54 & 296  & 252 & 78,57 & 10 & 246 & 246 & 296  & 252 & 2,38  & 9  \\
			alto-santo-3000-3                                                                 & 195 & 578  & 49  & 0 & 55 & 303  & 260 & 78,85 & 10 & 250 & 250 & 303  & 260 & 3,85  & 10 & 0 & 55 & 303  & 258 & 78,68 & 11 & 250 & 250 & 303  & 258 & 3,10  & 10 \\
			alto-santo-3000-4                                                                 & 195 & 578  & 49  & 0 & 54 & 310  & 267 & 79,78 & 10 & 257 & 257 & 310  & 267 & 3,75  & 9  & 0 & 54 & 310  & 265 & 79,62 & 11 & 257 & 257 & 310  & 265 & 3,02  & 9  \\
			alto-santo-3000-5                                                                 & 195 & 578  & 49  & 0 & 61 & 330  & 284 & 78,52 & 10 & 274 & 274 & 330  & 284 & 3,52  & 10 & 0 & 61 & 330  & 282 & 78,37 & 11 & 274 & 274 & 330  & 282 & 2,84  & 9  \\
			alto-santo-3000-6                                                                 & 195 & 578  & 53  & 0 & 79 & 399  & 337 & 76,56 & 10 & 324 & 324 & 399  & 337 & 3,86  & 10 & 0 & 79 & 399  & 335 & 76,42 & 11 & 324 & 324 & 399  & 335 & 3,28  & 10 \\
			\midrule
			limoeiro-1000-1                                                                   & 288 & 928  & 40  & 0 & 7  & 57   & 57  & 87,72 & 9  & 52  & 52  & 57   & 57  & 8,77  & 9  & 0 & 7  & 57   & 57  & 87,72 & 9  & 52  & 52  & 57   & 57  & 8,77  & 9  \\
			limoeiro-1000-2                                                                   & 306 & 984  & 49  & 0 & 12 & 79   & 74  & 83,78 & 11 & 65  & 65  & 79   & 74  & 12,16 & 10 & 0 & 13 & 79   & 72  & 81,94 & 11 & 65  & 65  & 79   & 72  & 9,72  & 10 \\
			limoeiro-1000-3                                                                   & 321 & 1050 & 68  & 0 & 27 & 139  & 118 & 77,12 & 12 & 108 & 108 & 139  & 118 & 8,47  & 11 & 0 & 27 & 139  & 117 & 76,92 & 12 & 108 & 108 & 139  & 117 & 7,69  & 10 \\
			limoeiro-1000-4                                                                   & 321 & 1050 & 73  & 0 & 27 & 145  & 120 & 77,50 & 12 & 110 & 110 & 145  & 120 & 8,33  & 10 & 0 & 27 & 145  & 118 & 77,12 & 11 & 110 & 110 & 145  & 118 & 6,78  & 10 \\
			limoeiro-1000-5                                                                   & 326 & 1068 & 85  & 0 & 33 & 189  & 151 & 78,15 & 11 & 136 & 136 & 189  & 151 & 9,93  & 10 & 0 & 33 & 189  & 149 & 77,85 & 11 & 136 & 136 & 189  & 149 & 8,72  & 11 \\
			limoeiro-1000-6                                                                   & 349 & 1148 & 142 & 0 & 63 & 671  & 441 & 85,71 & 12 & 390 & 390 & 671  & 441 & 11,56 & 12 & 0 & 63 & 671  & 431 & 85,38 & 13 & 390 & 390 & 671  & 431 & 9,51  & 12 \\
			limoeiro-1000-7                                                                   & 349 & 1148 & 146 & 0 & 79 & 719  & 468 & 83,12 & 13 & 414 & 414 & 719  & 468 & 11,54 & 12 & 0 & 79 & 719  & 458 & 82,75 & 12 & 414 & 414 & 719  & 458 & 9,61  & 12 \\
			limoeiro-2000-1                                                                   & 660 & 2112 & 79  & 0 & 8  & 110  & 77  & 89,61 & 15 & 59  & 59  & 110  & 77  & 23,38 & 14 & 0 & 8  & 110  & 75  & 89,33 & 16 & 59  & 59  & 110  & 75  & 21,33 & 14 \\
			limoeiro-2000-2                                                                   & 722 & 2336 & 110 & 0 & 16 & 184  & 114 & 85,96 & 16 & 89  & 89  & 184  & 114 & 21,93 & 15 & 0 & 16 & 184  & 111 & 85,59 & 17 & 89  & 89  & 184  & 111 & 19,82 & 16 \\
			limoeiro-2000-3                                                                   & 733 & 2388 & 142 & 0 & 27 & 274  & 159 & 83,02 & 21 & 132 & 132 & 274  & 159 & 16,98 & 14 & 0 & 27 & 274  & 155 & 82,58 & 20 & 132 & 132 & 274  & 155 & 14,84 & 20 \\
			limoeiro-2000-4                                                                   & 734 & 2394 & 149 & 0 & 27 & 288  & 163 & 83,44 & 19 & 131 & 131 & 288  & 163 & 19,63 & 14 & 0 & 27 & 288  & 159 & 83,02 & 19 & 131 & 131 & 288  & 159 & 17,61 & 19 \\
			limoeiro-2000-5                                                                   & 785 & 2564 & 189 & 0 & 33 & 404  & 204 & 83,82 & 23 & 157 & 157 & 404  & 204 & 23,04 & 20 & 0 & 33 & 404  & 199 & 83,42 & 24 & 157 & 157 & 404  & 199 & 21,11 & 19 \\
			limoeiro-2000-6                                                                   & 813 & 2652 & 325 & 0 & 63 & 1467 & 603 & 89,55 & 20 & 462 & 462 & 1467 & 603 & 23,38 & 19 & 0 & 63 & 1467 & 587 & 89,27 & 32 & 462 & 462 & 1467 & 587 & 21,29 & 31 \\
			limoeiro-2000-7                                                                   & 816 & 2662 & 334 & 0 & 79 & 1564 & 642 & 87,69 & 20 & 495 & 495 & 1564 & 642 & 22,90 & 19 & 0 & 79 & 1564 & 626 & 87,38 & 28 & 495 & 495 & 1564 & 626 & 20,93 & 24 \\
			limoeiro-3000-1                                                                   & 861 & 2744 & 87  & 0 & 8  & 121  & 78  & 89,74 & 17 & 58  & 58  & 121  & 78  & 25,64 & 16 & 0 & 8  & 121  & 76  & 89,47 & 17 & 58  & 58  & 121  & 76  & 23,68 & 17 \\
			limoeiro-3000-2                                                                   & 903 & 2902 & 120 & 0 & 16 & 199  & 114 & 85,96 & 18 & 87  & 87  & 199  & 114 & 23,68 & 18 & 0 & 16 & 199  & 111 & 85,59 & 22 & 87  & 87  & 199  & 111 & 21,62 & 19 \\
			limoeiro-3000-3                                                                   & 925 & 2982 & 154 & 0 & 27 & 294  & 159 & 83,02 & 21 & 127 & 127 & 294  & 159 & 20,13 & 14 & 0 & 27 & 294  & 156 & 82,69 & 26 & 127 & 127 & 294  & 156 & 18,59 & 20 \\
			limoeiro-3000-4                                                                   & 925 & 2982 & 161 & 0 & 27 & 312  & 163 & 83,44 & 19 & 132 & 132 & 312  & 163 & 19,02 & 15 & 0 & 27 & 312  & 160 & 83,12 & 24 & 132 & 132 & 312  & 160 & 17,50 & 23 \\
			limoeiro-3000-5                                                                   & 944 & 3048 & 202 & 0 & 33 & 446  & 205 & 83,90 & 22 & 156 & 156 & 446  & 205 & 23,90 & 21 & 0 & 28 & 446  & 200 & 86,00 & 27 & 156 & 156 & 446  & 200 & 22,00 & 22 \\
			limoeiro-3000-6                                                                   & 967 & 3130 & 345 & 0 & 61 & 1593 & 611 & 90,02 & 24 & 456 & 456 & 1593 & 611 & 25,37 & 20 & 0 & 63 & 1593 & 595 & 89,41 & 32 & 456 & 456 & 1593 & 595 & 23,36 & 26 \\
			limoeiro-3000-7                                                                   & 970 & 3140 & 356 & 0 & 79 & 1712 & 652 & 87,88 & 20 & 493 & 493 & 1712 & 652 & 24,39 & 21 & 0 & 79 & 1712 & 635 & 87,56 & 33 & 493 & 493 & 1712 & 635 & 22,36 & 30 \\
			\bottomrule
		\end{tabular}%
	}
\end{table}

Tables~\ref{tab:results-lagrange-relax-default} and~\ref{tab:results-lagrange-relax-preprocessing} present the computational results for the eight \gls{lr} configurations applied to the walk-based formulation. The first table examines four variants without preprocessing (LR-RCSP-KN, LR-RCSP-KN-HR, LR-RCSP-KN-BM, and LR-RCSP-KN-HR-BM), while the second table presents the same four configurations using the preprocessing. Each variant tracks both initial and final lower and upper bounds, providing insight into the convergence behavior of the subgradient optimization method.

The bounds analysis reveals distinct performance characteristics across the four configurations. Regarding \gls{lb}s, the heuristic-based variants (LR-RCSP-KN-HR and LR-RCSP-KN-HR-BM) achieve identical \gls{lb}s across all 39 instances, consistently outperforming both LR-RCSP-KN and LR-RCSP-KN-BM.
For \gls{ub}s, LR-RCSP-KN-BM and LR-RCSP-KN-HR-BM produce identical \gls{ub}s across all 39 instances, and both dominate LR-RCSP-KN and LR-RCSP-KN-HR. The LR-RCSP-KN-HR-BM variant achieves better or equal \gls{ub}s compared to LR-RCSP-KN-HR in all 39 instances, with strictly better \gls{ub}s in 36 instances and equal bounds in 3 instances.

The results demonstrate that the greedy heuristic is essential for generating strong \gls{lb}s, as both heuristic-based variants achieve values that are identical to or superior to those of non-heuristic variants.
Conversely, the barrier method initialization significantly improves \gls{ub} quality, as evidenced by LR-RCSP-KN-BM dominating LR-RCSP-KN in \gls{ub}s and LR-RCSP-KN-HR-BM consistently outperforming LR-RCSP-KN-HR.
The combination of both techniques in LR-RCSP-KN-HR-BM yields the best overall performance, achieving the strongest \gls{lb}s (tied with LR-RCSP-KN-HR) and the best \gls{ub}s (tied with LR-RCSP-KN-BM) across all instances.

The analysis of the four configurations with preprocessing reveals similar performance patterns to those without preprocessing, with some notable differences.
Regarding \gls{lb}s, the heuristic-based variants (LR-RCSP-KN-HR-Prep and LR-RCSP-KN-HR-BM-Prep) achieve identical \gls{lb}s across all 39 instances, consistently outperforming both LR-RCSP-KN-Prep and LR-RCSP-KN-BM-Prep.
Specifically, LR-RCSP-KN-HR-Prep and LR-RCSP-KN-HR-BM-Prep dominate LR-RCSP-KN-Prep and LR-RCSP-KN-BM-Prep for every instance.
For \gls{ub}s, LR-RCSP-KN-BM-Prep and LR-RCSP-KN-HR-BM-Prep produce identical \gls{ub}s across all 39 instances, and both dominate LR-RCSP-KN-Prep and LR-RCSP-KN-HR-Prep.

Comparing the best-performing configurations from each table, LR-RCSP-KN-HR-BM and LR-RCSP-KN-HR-BM-Prep, across the 39 common instances reveals that preprocessing does not provide a clear advantage for all bounds. For \gls{lb}s, the version without preprocessing achieves better \gls{lb}s in 10 instances, the version with preprocessing in 7 instances, and both achieve equal \gls{lb}s in 22 instances, indicating no dominance in either direction. For \gls{ub}s, both configurations produce identical \gls{ub}s across all 39 instances, demonstrating that preprocessing does not impact the \gls{ub} quality for the \gls{lr} method in this set of instances, however the value of the \gls{lb} is improved in some cases.

\paragraph{Comparison between LR-RCSP-KN-HR-BM-Prep and Linear Relaxation}

To validate the quality of a Lagrangean Relaxation, a valid approach is to compare it with the Linear Relaxation of the complete original problem. The Linear Relaxation is obtained by flexibilizing the integer variables of the original problem to be real-valued. The Linear Relaxation is a much easier problem to solve than the original problem and is a good approximation.

Since the comparison consider the \gls{ub}s and the \gls{lr} with and without preprocessing achieved the same values, the selected version to compare is LR-RCSP-KN-HR-BM-Prep. Table~\ref{tab:results-lagrange-vs-linear} presents a comparison between LR-RCSP-KN-HR-BM-Prep and the linear relaxation of Walk-CBRP-Prep with the addition of the route and block attending time constraints.
LR-RCSP-KN-HR-BM-Prep achieves better values in 36 instances, while both methods are equal in 3 instances.
These results demonstrate that the \gls{lr} approach consistently outperforms the linear relaxation in terms of \gls{ub} quality across the instance set.
Considering the execution time, the linear relaxation is significantly faster than the \gls{lr}; however, the \gls{lr} does not exceed 90 seconds and scales better with instance size due to not requiring a solver.

\begin{table}[ht!]
	\centering
	\caption{Comparison between LR-RCSP-KN-HR-BM-Prep and Walk-CBRP-Prep - Linear Relaxation.}
	\label{tab:results-lagrange-vs-linear}
	{\tiny%
		\begin{tabular}{lrrr|rr|rr}
			...
			\multicolumn{1}{c}{\textbf{|A|}}                                                                     &
			\multicolumn{1}{c|}{\textbf{|B|}}                                                                    &
			\multicolumn{2}{c|}{\textbf{\begin{tabular}[c]{@{}c@{}}Best UB\\Lagrangean Relaxation\end{tabular}}} &
			\multicolumn{2}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Walk-CBRP-Prep\\Linear Relaxation\end{tabular}}}                                              \\
			...
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Exec. Time (s)\end{tabular}}}                                                                 \\
			\midrule
			alto-santo-1000-1                                                                                    & 179  & 524  & 84  & 39  & 0  & 39,00  & 0,00 \\
			alto-santo-1000-2                                                                                    & 179  & 524  & 84  & 243 & 22 & 243,77 & 0,00 \\
			alto-santo-1000-3                                                                                    & 179  & 524  & 84  & 250 & 21 & 250,77 & 0,00 \\
			alto-santo-1000-4                                                                                    & 179  & 524  & 84  & 257 & 21 & 257,77 & 0,00 \\
			alto-santo-1000-5                                                                                    & 179  & 524  & 84  & 273 & 21 & 273,77 & 0,00 \\
			alto-santo-1000-6                                                                                    & 179  & 524  & 84  & 328 & 22 & 328,24 & 0,00 \\
			alto-santo-2000-1                                                                                    & 253  & 710  & 102 & 39  & 0  & 39,00  & 0,00 \\
			alto-santo-2000-2                                                                                    & 253  & 710  & 102 & 252 & 23 & 252,66 & 0,00 \\
			alto-santo-2000-3                                                                                    & 253  & 710  & 102 & 258 & 21 & 259,17 & 0,00 \\
			alto-santo-2000-4                                                                                    & 253  & 710  & 102 & 265 & 22 & 266,17 & 0,00 \\
			alto-santo-2000-5                                                                                    & 253  & 710  & 102 & 282 & 22 & 283,17 & 0,00 \\
			alto-santo-2000-6                                                                                    & 253  & 710  & 102 & 335 & 22 & 335,41 & 0,00 \\
			alto-santo-3000-1                                                                                    & 353  & 964  & 128 & 40  & 23 & 40,98  & 0,00 \\
			alto-santo-3000-2                                                                                    & 353  & 964  & 128 & 252 & 23 & 252,67 & 0,00 \\
			alto-santo-3000-3                                                                                    & 353  & 964  & 128 & 258 & 24 & 259,18 & 0,00 \\
			alto-santo-3000-4                                                                                    & 353  & 964  & 128 & 265 & 24 & 266,18 & 0,00 \\
			alto-santo-3000-5                                                                                    & 353  & 964  & 128 & 282 & 25 & 283,18 & 0,00 \\
			alto-santo-3000-6                                                                                    & 353  & 964  & 128 & 335 & 23 & 335,42 & 0,00 \\
			\midrule
			limoeiro-1000-1                                                                                      & 400  & 1260 & 231 & 57  & 21 & 57,00  & 0,01 \\
			limoeiro-1000-2                                                                                      & 400  & 1260 & 231 & 72  & 21 & 72,82  & 0,01 \\
			limoeiro-1000-3                                                                                      & 400  & 1260 & 231 & 117 & 22 & 117,16 & 0,01 \\
			limoeiro-1000-4                                                                                      & 400  & 1260 & 231 & 118 & 22 & 118,47 & 0,01 \\
			limoeiro-1000-5                                                                                      & 400  & 1260 & 231 & 149 & 26 & 149,40 & 0,01 \\
			limoeiro-1000-6                                                                                      & 400  & 1260 & 231 & 431 & 29 & 432,44 & 0,01 \\
			limoeiro-1000-7                                                                                      & 400  & 1260 & 231 & 458 & 27 & 459,36 & 0,01 \\
			limoeiro-2000-1                                                                                      & 1008 & 3112 & 549 & 75  & 33 & 75,42  & 0,03 \\
			limoeiro-2000-2                                                                                      & 1008 & 3112 & 549 & 111 & 39 & 111,62 & 0,01 \\
			limoeiro-2000-3                                                                                      & 1008 & 3112 & 549 & 155 & 41 & 156,00 & 0,02 \\
			limoeiro-2000-4                                                                                      & 1008 & 3112 & 549 & 159 & 42 & 160,25 & 0,01 \\
			limoeiro-2000-5                                                                                      & 1008 & 3112 & 549 & 199 & 48 & 200,17 & 0,02 \\
			limoeiro-2000-6                                                                                      & 1008 & 3112 & 549 & 587 & 70 & 588,35 & 0,01 \\
			limoeiro-2000-7                                                                                      & 1008 & 3112 & 549 & 626 & 70 & 626,14 & 0,02 \\
			limoeiro-3000-1                                                                                      & 1240 & 3798 & 660 & 76  & 40 & 76,16  & 0,03 \\
			limoeiro-3000-2                                                                                      & 1240 & 3798 & 660 & 111 & 45 & 111,68 & 0,02 \\
			limoeiro-3000-3                                                                                      & 1240 & 3798 & 660 & 156 & 54 & 156,31 & 0,01 \\
			limoeiro-3000-4                                                                                      & 1240 & 3798 & 660 & 160 & 57 & 160,53 & 0,01 \\
			limoeiro-3000-5                                                                                      & 1240 & 3798 & 660 & 200 & 54 & 201,09 & 0,01 \\
			limoeiro-3000-6                                                                                      & 1240 & 3798 & 660 & 595 & 65 & 595,49 & 0,02 \\
			limoeiro-3000-7                                                                                      & 1240 & 3798 & 660 & 635 & 63 & 635,51 & 0,02 \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
This section provides an analysis of the lower and upper bounds across all approaches that generate walk solutions. Table~\ref{tab:results-all-lower-bounds} presents a comparison of the \gls{lb}s for the Greedy Heuristic, the best Lagrangean Relaxation and the Walk-Based Formulations.

\begin{table}[ht!]
	\centering
	\caption{Comparison of Lower Bounds across all approaches.}
	\label{tab:results-all-lower-bounds}
	{\tiny%
		\begin{tabular}{lrrrrrr}
			\toprule
			\multicolumn{1}{c}{\textbf{Instance}}                                                   &
			\multicolumn{1}{c}{\textbf{\begin{tabular}[c]{@{}c@{}}Greedy\\ Heuristic\end{tabular}}} &
			\multicolumn{1}{c}{\textbf{LR-RCSP-KN-HR-BM}}                                           &
			\multicolumn{1}{c}{\textbf{LR-RCSP-KN-HR-BM-Prep}}                                      &
			\multicolumn{1}{c}{\textbf{Walk-CBRP}}                                                  &
			\multicolumn{1}{c}{\textbf{Walk-CBRP-Prep}}                                             &
			\multicolumn{1}{c}{\textbf{Walk-CBRP-MTZ}}                                                                                  \\
			\midrule
			alto-santo-1000-1                                                                       & 39  & 39  & 39  & 39  & 39  & 39  \\
			alto-santo-1000-2                                                                       & 238 & 238 & 238 & 49  & 238 & 237 \\
			alto-santo-1000-3                                                                       & 245 & 245 & 245 & 50  & 246 & 240 \\
			alto-santo-1000-4                                                                       & 252 & 252 & 252 & 50  & 253 & 217 \\
			alto-santo-1000-5                                                                       & 268 & 268 & 268 & 111 & 269 & 257 \\
			alto-santo-1000-6                                                                       & 319 & 318 & 319 & 61  & 319 & 310 \\
			alto-santo-2000-1                                                                       & 39  & 39  & 39  & 39  & 39  & 39  \\
			alto-santo-2000-2                                                                       & 247 & 246 & 247 & 246 & 246 & 233 \\
			alto-santo-2000-3                                                                       & 251 & 251 & 251 & 21  & 252 & 229 \\
			alto-santo-2000-4                                                                       & 258 & 258 & 258 & 256 & 259 & 253 \\
			alto-santo-2000-5                                                                       & 275 & 275 & 275 & 276 & 276 & 231 \\
			alto-santo-2000-6                                                                       & 324 & 324 & 324 & 35  & 325 & 296 \\
			alto-santo-3000-1                                                                       & 39  & 39  & 39  & 2   & 39  & 38  \\
			alto-santo-3000-2                                                                       & 246 & 246 & 246 & 247 & 247 & 197 \\
			alto-santo-3000-3                                                                       & 251 & 251 & 250 & 251 & 251 & 214 \\
			alto-santo-3000-4                                                                       & 258 & 258 & 257 & 259 & 258 & 231 \\
			alto-santo-3000-5                                                                       & 275 & 275 & 274 & 275 & 275 & 272 \\
			alto-santo-3000-6                                                                       & 324 & 323 & 324 & 326 & 91  & 310 \\
			\midrule
			limoeiro-1000-1                                                                         & 52  & 51  & 52  & 8   & 8   & 47  \\
			limoeiro-1000-2                                                                         & 65  & 65  & 65  & 6   & 64  & 57  \\
			limoeiro-1000-3                                                                         & 108 & 108 & 108 & 24  & 23  & 8   \\
			limoeiro-1000-4                                                                         & 110 & 110 & 110 & 8   & 30  & 2   \\
			limoeiro-1000-5                                                                         & 136 & 136 & 136 & 42  & 12  & 7   \\
			limoeiro-1000-6                                                                         & 390 & 389 & 390 & 42  & 33  & 11  \\
			limoeiro-1000-7                                                                         & 415 & 415 & 414 & 59  & 35  & 13  \\
			limoeiro-2000-1                                                                         & 59  & 59  & 59  & 8   & 4   & 6   \\
			limoeiro-2000-2                                                                         & 89  & 87  & 89  & 13  & 7   & 6   \\
			limoeiro-2000-3                                                                         & 132 & 129 & 132 & 27  & 30  & 6   \\
			limoeiro-2000-4                                                                         & 132 & 132 & 131 & 23  & 8   & 5   \\
			limoeiro-2000-5                                                                         & 157 & 157 & 157 & 33  & 13  & -   \\
			limoeiro-2000-6                                                                         & 471 & 471 & 462 & 107 & 41  & -   \\
			limoeiro-2000-7                                                                         & 496 & 496 & 495 & 48  & 91  & -   \\
			limoeiro-3000-1                                                                         & 58  & 58  & 58  & 5   & 8   & 2   \\
			limoeiro-3000-2                                                                         & 89  & 89  & 87  & 21  & 14  & -   \\
			limoeiro-3000-3                                                                         & 127 & 127 & 127 & 27  & 24  & -   \\
			limoeiro-3000-4                                                                         & 132 & 130 & 132 & 8   & 30  & -   \\
			limoeiro-3000-5                                                                         & 156 & 156 & 156 & 28  & 10  & -   \\
			limoeiro-3000-6                                                                         & 457 & 457 & 456 & 58  & 85  & -   \\
			limoeiro-3000-7                                                                         & 493 & 493 & 493 & 79  & 122 & -   \\
			\bottomrule
		\end{tabular}%
	}
\end{table}

Considering the results presented in Table~\ref{tab:results-all-lower-bounds}, both the Greedy Heuristic and the best Lagrangean Relaxation achieve the same, or very similar, \gls{lb}s for all instances. This result reinforces the quality of the Greedy Heuristic in generating strong \gls{lb}s.
The Walk-Based Formulations (with and without preprocessing) achieved \gls{lb}s that are better than the Greedy Heuristic for the majority of Alto Santo instances (11 out of 18), however as the size of the instances increases, the quality of the \gls{lb}s achieved by the Walk-Based Formulations decreases. For the 21 instances of Limoeiro, the walk-based formulations achieved \gls{lb}s inferior to the Greedy Heuristic and consequently the \gls{lr}s. The Walk-CBRP-MTZ achieve inferior bounds for all the instances.

Figure~\ref{fig:cd-walk-lb} presents the Iman-Davenport and Nemenyi tests for the lower bounds across all approaches. The test indicates that the Greedy Heuristic, \gls{lr}s and the Walk-CBRP-Prep are statistically equivalent (lowest average ranks). For those four approaches, the tiebreaker is the win count, presented in Table~\ref{tab:wins-equiv-lb}. Based on the win count, the LR-RCSP-KN-HR-BM-Prep achieved the best results, with 37 wins, followed by the Greedy Heuristic and the LR-RCSP-KN-HR-BM, with 32 wins.

\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.35]{images/deterministic-experiments/cd_csv_walk_lb.pdf}
	\caption{Iman-Davenport and Nemenyi tests for Lower Bounds: ID = 29,16; CV = 2,26; CD = 1,21} \label{fig:cd-walk-lb}
\end{figure}

\begin{table}[ht!]
	\centering
	\caption{Win counts for statistically equivalent \gls{lb} approaches.}
	\label{tab:wins-equiv-lb}
	\resizebox{\textwidth}{!}{%
		\centering
		\begin{tabular}{lrrrr}
			\hline
			                      & Greedy-Heuristic & LR-RCSP-KN-HR-BM & LR-RCSP-KN-HR-BM-Prep & Walk-CBRP-Prep \\
			\hline
			Greedy-Heuristic      & -                & 0                & 10                    & 22             \\
			LR-RCSP-KN-HR-BM      & 0                & -                & 10                    & 22             \\
			LR-RCSP-KN-HR-BM-Prep & 7                & 7                & -                     & 23             \\
			Walk-CBRP-Prep        & 9                & 9                & 12                    & -              \\
			\hline
		\end{tabular}
	}
\end{table}

Table~\ref{tab:results-all-upper-bounds} presents a comparison of the \gls{ub}s for the best \gls{lr} and the Walk-Based Formulations. The Iman-Davenport and Nemenyi tests are presented in Figure~\ref{fig:cd-walk-ub}, which indicated that Walk-CBRP-Prep and Walk-CBRP are statistically equivalent as the best approaches to generate \gls{ub}s.
Based on the win count, Walk-CBRP achieved better results for 6 instances, Walk-CBRP-Prep achieved better results for 17, and for 16 instances both approaches achieved the same \gls{ub}s. Based on these results, Walk-CBRP-Prep can be considered the best approach to generate \gls{ub}s for the Walk-Based methodologies.

\begin{table}[ht!]
	\centering
	\caption{Comparison of Upper Bounds across all approaches.}
	\label{tab:results-all-upper-bounds}
	{\tiny%
		\begin{tabular}{lrrrr}
			\toprule
			\multicolumn{1}{c}{\textbf{Instance}}              &
			...
			alto-santo-1000-2                                  & 243 & 243 & 241 & 242 \\
			alto-santo-1000-3                                  & 250 & 250 & 248 & 250 \\
			alto-santo-1000-4                                  & 257 & 257 & 255 & 256 \\
			alto-santo-1000-5                                  & 273 & 272 & 271 & 273 \\
			alto-santo-1000-6                                  & 328 & 324 & 323 & 326 \\
			alto-santo-2000-1                                  & 39  & 39  & 39  & 39  \\
			alto-santo-2000-2                                  & 252 & 249 & 249 & 251 \\
			alto-santo-2000-3                                  & 258 & 256 & 255 & 257 \\
			alto-santo-2000-4                                  & 265 & 262 & 262 & 265 \\
			alto-santo-2000-5                                  & 282 & 279 & 279 & 282 \\
			alto-santo-2000-6                                  & 335 & 330 & 330 & 333 \\
			alto-santo-3000-1                                  & 40  & 40  & 40  & 40  \\
			alto-santo-3000-2                                  & 252 & 251 & 249 & 251 \\
			alto-santo-3000-3                                  & 258 & 258 & 255 & 258 \\
			alto-santo-3000-4                                  & 265 & 265 & 265 & 265 \\
			alto-santo-3000-5                                  & 282 & 282 & 279 & 281 \\
			alto-santo-3000-6                                  & 335 & 334 & 330 & 333 \\
			\midrule
			limoeiro-1000-1                                    & 57  & 56  & 56  & 57  \\
			limoeiro-1000-2                                    & 72  & 70  & 69  & 71  \\
			limoeiro-1000-3                                    & 117 & 117 & 113 & 116 \\
			limoeiro-1000-4                                    & 118 & 116 & 114 & 118 \\
			limoeiro-1000-5                                    & 149 & 149 & 146 & 149 \\
			limoeiro-1000-6                                    & 431 & 422 & 413 & -   \\
			limoeiro-1000-7                                    & 458 & 458 & 438 & -   \\
			limoeiro-2000-1                                    & 75  & 71  & 72  & -   \\
			limoeiro-2000-2                                    & 111 & 111 & 107 & -   \\
			limoeiro-2000-3                                    & 155 & 149 & 155 & -   \\
			limoeiro-2000-4                                    & 159 & 159 & 159 & -   \\
			limoeiro-2000-5                                    & 199 & 191 & 191 & -   \\
			limoeiro-2000-6                                    & 587 & 560 & 562 & -   \\
			limoeiro-2000-7                                    & 626 & 601 & 625 & -   \\
			limoeiro-3000-1                                    & 76  & 76  & 76  & -   \\
			limoeiro-3000-2                                    & 111 & 106 & 106 & -   \\
			limoeiro-3000-3                                    & 156 & 149 & 156 & -   \\
			limoeiro-3000-4                                    & 160 & 160 & 160 & -   \\
			limoeiro-3000-5                                    & 200 & 192 & 192 & -   \\
			limoeiro-3000-6                                    & 595 & 568 & 594 & -   \\
			limoeiro-3000-7                                    & 635 & 613 & 606 & -   \\
			\bottomrule
		\end{tabular}%
	}
\end{table}
...
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.35]{images/deterministic-experiments/cd_csv_walk_ub.pdf}
	\caption{Iman-Davenport and Nemenyi tests for Upper Bounds: ID = 28,27; CV = 2,26; CD = 1,21} \label{fig:cd-walk-ub}
\end{figure}

\subsection{Path vs Walk Analysis}\label{subsec:path-vs-walk-analysis}

Since the Walk-Based Formulations are a generalization of the Path-Based Formulations, allowing the use of more flexible paths considering repetitions of arcs, the goal is to identify the gain of using the Walk-Based Formulations compared to the Path-Based Formulations. For this analysis, the best bounds were considered, independent of the approach used to generate them.

Table~\ref{tab:all-bounds-analysis} presents a comparison of the \gls{lb}s and \gls{ub}s for the Path-Based and Walk-Based Formulations.
The first three columns present the bounds and GAP for the Path-Based approaches, the next three columns present the bounds and GAP for the Walk-Based approaches, and the last two columns present the gap between the \gls{lb} (\gls{ub}) of the Path-Based Formulations and the \gls{lb} (\gls{ub}) of the Walk-Based Formulations.

\begin{table}[ht!]
	\centering
	\caption{All Bounds Analysis - Path and Walk Comparison}
	\label{tab:all-bounds-analysis}
	{\tiny%
		\begin{tabular}{lrrrrrrrr}
			\hline
			                                      &
			\multicolumn{3}{c}{\textbf{Path}}     &
			\multicolumn{3}{c}{\textbf{Walk}}     &
			\multicolumn{1}{l}{}                  &
			\multicolumn{1}{l}{}                                                                          \\ \cline{2-7}
			\multicolumn{1}{c}{\textbf{Instance}} &
			\multicolumn{1}{l}{Best LB}           &
			\multicolumn{1}{l}{Best UB}           &
			\multicolumn{1}{l}{GAP (\%)}          &
			\multicolumn{1}{l}{Best LB}           &
			\multicolumn{1}{l}{Best UB}           &
			\multicolumn{1}{l}{GAP (\%)}          &
			\multicolumn{1}{l}{LB GAP (\%)}       &
			\multicolumn{1}{l}{UB GAP (\%)}                                                               \\ \hline
			alto-santo-1000-1                     & 39  & 39  & 0,00 & 39  & 39  & 0,00  & 0,00   & 0,00  \\
			alto-santo-1000-2                     & 240 & 240 & 0,00 & 238 & 241 & 1,24  & -0,84  & 0,41  \\
			alto-santo-1000-3                     & 247 & 247 & 0,00 & 246 & 248 & 0,81  & -0,41  & 0,40  \\
			alto-santo-1000-4                     & 254 & 254 & 0,00 & 253 & 255 & 0,78  & -0,40  & 0,39  \\
			alto-santo-1000-5                     & 270 & 270 & 0,00 & 269 & 271 & 0,74  & -0,37  & 0,37  \\
			alto-santo-1000-6                     & 321 & 321 & 0,00 & 319 & 323 & 1,24  & -0,63  & 0,62  \\
			alto-santo-2000-1                     & 39  & 39  & 0,00 & 39  & 39  & 0,00  & 0,00   & 0,00  \\
			alto-santo-2000-2                     & 248 & 248 & 0,00 & 247 & 249 & 0,80  & -0,40  & 0,40  \\
			alto-santo-2000-3                     & 253 & 253 & 0,00 & 252 & 255 & 1,18  & -0,40  & 0,78  \\
			alto-santo-2000-4                     & 260 & 260 & 0,00 & 259 & 262 & 1,15  & -0,39  & 0,76  \\
			alto-santo-2000-5                     & 277 & 277 & 0,00 & 276 & 279 & 1,08  & -0,36  & 0,72  \\
			alto-santo-2000-6                     & 327 & 327 & 0,00 & 325 & 330 & 1,52  & -0,62  & 0,91  \\
			alto-santo-3000-1                     & 39  & 39  & 0,00 & 39  & 40  & 2,50  & 0,00   & 2,50  \\
			alto-santo-3000-2                     & 248 & 248 & 0,00 & 247 & 249 & 0,80  & -0,40  & 0,40  \\
			alto-santo-3000-3                     & 253 & 253 & 0,00 & 251 & 255 & 1,57  & -0,80  & 0,78  \\
			alto-santo-3000-4                     & 260 & 260 & 0,00 & 259 & 265 & 2,26  & -0,39  & 1,89  \\
			alto-santo-3000-5                     & 277 & 277 & 0,00 & 275 & 279 & 1,43  & -0,73  & 0,72  \\
			alto-santo-3000-6                     & 327 & 327 & 0,00 & 326 & 330 & 1,21  & -0,31  & 0,91  \\ \hline
			limoeiro-1000-1                       & 53  & 53  & 0,00 & 51  & 56  & 8,93  & -3,92  & 5,36  \\
			limoeiro-1000-2                       & 67  & 68  & 1,47 & 65  & 69  & 5,80  & -3,08  & 1,45  \\
			limoeiro-1000-3                       & 110 & 111 & 0,90 & 108 & 113 & 4,42  & -1,85  & 1,77  \\
			limoeiro-1000-4                       & 111 & 112 & 0,89 & 110 & 114 & 3,51  & -0,91  & 1,75  \\
			limoeiro-1000-5                       & 139 & 141 & 1,42 & 137 & 146 & 6,16  & -1,46  & 3,42  \\
			limoeiro-1000-6                       & 398 & 403 & 1,24 & 390 & 413 & 5,57  & -2,05  & 2,42  \\
			limoeiro-1000-7                       & 424 & 430 & 1,40 & 415 & 438 & 5,25  & -2,17  & 1,83  \\
			limoeiro-2000-1                       & 63  & 67  & 5,97 & 59  & 71  & 16,90 & -6,78  & 5,63  \\
			limoeiro-2000-2                       & 95  & 101 & 5,94 & 89  & 107 & 16,82 & -6,74  & 5,61  \\
			limoeiro-2000-3                       & 136 & 143 & 4,90 & 132 & 149 & 11,41 & -3,03  & 4,03  \\
			limoeiro-2000-4                       & 138 & 146 & 5,48 & 132 & 159 & 16,98 & -4,55  & 8,18  \\
			limoeiro-2000-5                       & 173 & 183 & 5,46 & 157 & 191 & 17,80 & -10,19 & 4,19  \\
			limoeiro-2000-6                       & 500 & 534 & 6,37 & 471 & 560 & 15,89 & -6,16  & 4,64  \\
			limoeiro-2000-7                       & 527 & 569 & 7,38 & 496 & 601 & 17,47 & -6,25  & 5,32  \\
			limoeiro-3000-1                       & 63  & 68  & 7,35 & 58  & 76  & 23,68 & -8,62  & 10,53 \\
			limoeiro-3000-2                       & 94  & 101 & 6,93 & 89  & 106 & 16,04 & -5,62  & 4,72  \\
			limoeiro-3000-3                       & 135 & 144 & 6,25 & 127 & 149 & 14,77 & -6,30  & 3,36  \\
			limoeiro-3000-4                       & 138 & 147 & 6,12 & 132 & 160 & 17,50 & -4,55  & 8,13  \\
			limoeiro-3000-5                       & 170 & 184 & 7,61 & 156 & 192 & 18,75 & -8,97  & 4,17  \\
			limoeiro-3000-6                       & 498 & 542 & 8,12 & 457 & 568 & 19,54 & -8,97  & 4,58  \\
			limoeiro-3000-7                       & 539 & 581 & 7,23 & 493 & 606 & 18,65 & -9,33  & 4,13  \\ \hline
		\end{tabular}%
	}
\end{table}

The results presented in Table~\ref{tab:all-bounds-analysis} reveal important differences between the Path-Based and Walk-Based Formulations. Across all 39 instances, the Walk-Based approaches consistently produce weaker bounds than their Path-Based counterparts. The average \gls{lb} GAP is $-3.05\%$, indicating that the best Walk \gls{lb} is, on average, $3.05\%$ lower than the best Path \gls{lb}. Similarly, the average \gls{ub} GAP is $2.77\%$, meaning the Walk \gls{ub} exceeds the Path \gls{ub} by $2.77\%$ on average. Notably, in no instance does the Walk-Based Formulation achieve a better (higher) \gls{lb} or a tighter (lower) \gls{ub} than the Path-Based Formulation. As a consequence, the average optimality GAP increases from $2.52\%$ for the Path-Based Formulations to $7.75\%$ for the Walk-Based Formulations. The Path-Based approaches achieve proven optimality on 19 out of 39 instances, whereas the Walk-Based approaches achieve this on only 2 instances.

A clear scalability tendency emerges when analyzing the results by instance size. For the Limoeiro instances, the average Path GAP grows from $1.05\%$ (-1000- instances) to $5.93\%$ (-2000- instances) and $7.09\%$ (-3000- instances), while the Walk GAP increases more steeply from $5.66\%$ to $16.18\%$ and $18.42\%$, respectively. This indicates that the Walk-Based Formulation's relaxation weakens at a faster rate as instances grow in size. A similar pattern is observed for Alto Santo, where the Walk GAP increases from $0.80\%$ (-1000- instances) to $0.96\%$ (-2000- instances) and $1.63\%$ (-3000- instances), even though the Path-Based Formulation maintains optimality across all sizes.

These results are consistent with the theoretical expectation that generalizing from paths to walks enlarges the feasible region, thereby increasing the difficulty of the problem, as confirmed by the increase in the GAP and the reduction in the average value of the \gls{lb}.
Since every path is also a walk, the walk methodologies admit additional solutions, reinforced by the \gls{ub} values for this maximization problem, thus yielding the possibility of increasing the solution value, on average, by $0.72\%$ for the Alto Santo and $4.53\%$ for the Limoeiro instances, which also tends to scale with instance size.
Furthermore, the larger solution space makes it more challenging for primal heuristics to identify high-quality feasible walk solutions, which explains the systematic degradation in \gls{lb}s.
Despite these limitations, the Walk-Based Formulation remains of interest because it offers modeling flexibility for scenarios where arc repetitions may be necessary or beneficial in practice, which is the case of block attending.