...
In our experiments, the use of the preprocessing steps for the Walk-based formulation achieved significant improvements in the solution quality, it is natural to extend this approach to the walk-based formulation. In addiction, the use of fractional cuts for the path-based formulation proved to be detrimental to the solution quality and will not be used in the walk-based formulation. Considering that the results achieved with fractional cuts and without preprocessing were fully dominated by the results with preprocessing and without fractional cuts considering both lower and upper bounds, those results will not be presented. With this, two variants of the walk-based formulation will be evaluated.
...
Considering the results presented in Table~\ref{tab:results-det-walk-cbrp-prep-exp}, the Walk-CBRP-Prep formulation achieved two optimal solutions for the Alto Santo instances. These instances represent the smallest problem configurations with only 19 blocks each. All other Alto Santo instances showed small gaps ranging from 0.74\% to 2.64\%, except for the instance alto-santo-3000-6 which presented a significantly larger gap of 72.42\%, indicating substantial difficulty in solving this particular configuration, which may be attributed to specific cases distribution that challenge the approach to find feasible solutions. The Limoeiro instances presented considerably larger gaps for the Walk-CBRP-Prep formulation, with values ranging from 7.25\% to 94.79\%. The smaller Limoeiro instances (limoeiro-1000-2) achieved the best results with a gap of 7.25\%, while the majority of instances exhibited gaps above 80\%. This huge contrast with Alto Santo results highlights the increased complexity of the Limoeiro urban structure, which presents a significantly larger road network with more vertices (285--952 versus 98--195), arcs (872--3067 versus 282--566), and blocks (40--356 versus 19--53). The execution times for most instances reached the 1 hour time limit, indicating that the solver struggled to improve solutions or close the optimality gap within the allocated computational budget.

Considering the lazy cuts column, it is possible to observe that the Walk-CBRP-Prep formulation spent significant computational effort detecting and eliminating infeasible solutions through the lazy constraint callback mechanism. For Alto Santo instances that achieved optimal or near-optimal solutions, the number of lazy cuts ranged from 93,964 to 1,016,942, with smaller instances generally requiring fewer cuts. In contrast, Limoeiro instances generated substantially more lazy cuts (360,954 to 1,624,492), reflecting the increased difficulty in enforcing subtour elimination constraints on these larger and more complex networks. The high number of lazy cuts combined with reaching the time limit suggests that the formulation spends significant computational effort detecting and eliminating infeasible solutions through the lazy constraint callback mechanism.

Considering the results presented in Table~\ref{tab:results-det-walk-cbrp-mtz}, the Walk-CBRP-MTZ formulation achieved two optimal solutions for the Alto Santo instances, matching the Walk-CBRP-Prep performance on these smallest instances. The remaining Alto Santo instances showed gaps ranging from 2.07\% to 21.51\%, generally larger than those obtained by Walk-CBRP-Prep. For Limoeiro instances, the Walk-CBRP-MTZ formulation encountered significant challenges, with gaps ranging from 17.54\% to 98.31\%. More critically, several instances (limoeiro-1000-6, limoeiro-1000-7, and multiple instances from the 2000 and 3000 series) failed to find feasible upper bounds within the time limit, as indicated by the dashes in the columns. The failure to obtain any bound is due the hugh memory consumption required to solve the problem, exceeding 50Gb of memory before solve even the root node of the branch and bound tree.
...
Six variations of the LR-RCSP-KN will be evaluated, using as base the version \textsc{LR-RCSP-KN}, that uses the Boost algorithm to compute \gls{rcsp} problems and the dynamic programming algorithm to compute \gls{kn} problems, all the lagrange multipliers are initialized with 0 and makes no use of preprocessing or heuristics to improve the solution. The variations include in the name the following suffixes:
...
Considering the essential parameters used in the subgradient method, the first is the the maximum execution time as a stoppint criteria, limiting the execution to 1800 seconds (30 minutes). The following combinations of values were evaluated:
...
Tables~\ref{tab:results-lagrange-relax-default} and~\ref{tab:results-lagrange-relax-preprocessing} present the computational results for eight configurations of Lagrangean Relaxation applied to the walk-based formulation. The first table examines four variants without preprocessing (LR-RCSP-KN, LR-RCSP-KN-HR, LR-RCSP-KN-BM, and LR-RCSP-KN-HR-BM), while the second table presents the same four configurations enhanced with preprocessing steps. Each variant tracks both initial and final lower and upper bounds, providing insight into the convergence behavior of the subgradient optimization method.

The analysis of lower and upper bounds reveals distinct performance characteristics across the four configurations. Regarding \gls{lb}s, the heuristic-based variants (LR-RCSP-KN-HR and LR-RCSP-KN-HR-BM) achieve identical \gls{lb}s across all 39 instances, consistently outperforming both LR-RCSP-KN and LR-RCSP-KN-BM. Specifically, LR-RCSP-KN-HR and LR-RCSP-KN-HR-BM dominate LR-RCSP-KN and LR-RCSP-KN-BM for every instance. The comparison between LR-RCSP-KN and LR-RCSP-KN-BM shows no clear dominance: LR-RCSP-KN produces better \gls{lb}s in 5 instances, LR-RCSP-KN-BM in 5 instances, and both achieve equal \gls{lb}s in 29 instances. For \gls{ub}s, LR-RCSP-KN-BM and LR-RCSP-KN-HR-BM produce identical \gls{ub}s across all 39 instances, and both dominate LR-RCSP-KN and LR-RCSP-KN-HR. The LR-RCSP-KN-HR-BM variant achieves better or equal \gls{ub}s compared to LR-RCSP-KN-HR in all 39 instances, with strictly better \gls{ub}s in 36 instances and equal bounds in 3 instances. When comparing LR-RCSP-KN and LR-RCSP-KN-BM, the barrier method variant demonstrates clear superiority: LR-RCSP-KN-BM produces better \gls{ub}s in 36 instances and equal bounds in 3 instances, while LR-RCSP-KN never achieves a better \gls{ub}.

The results demonstrate that the greedy heuristic is essential for generating strong \gls{lb}s, as both heuristic-based variants achieve identical and superior \gls{lb}s compared to non-heuristic variants. Conversely, the barrier method initialization significantly improves \gls{ub} quality, as evidenced by LR-RCSP-KN-BM dominating LR-RCSP-KN in \gls{ub}s and LR-RCSP-KN-HR-BM consistently outperforming LR-RCSP-KN-HR. The combination of both techniques in LR-RCSP-KN-HR-BM yields the best overall performance, achieving the strongest \gls{lb}s (tied with LR-RCSP-KN-HR) and the best \gls{ub}s (tied with LR-RCSP-KN-BM) across all instances.

The analysis of the four configurations with preprocessing reveals similar performance patterns to those without preprocessing, with some notable differences. Regarding \gls{lb}s, the heuristic-based variants (LR-RCSP-KN-HR-Prep and LR-RCSP-KN-HR-BM-Prep) achieve identical \gls{lb}s across all 39 instances, consistently outperforming both LR-RCSP-KN-Prep and LR-RCSP-KN-BM-Prep. Specifically, LR-RCSP-KN-HR-Prep and LR-RCSP-KN-HR-BM-Prep dominate LR-RCSP-KN-Prep and LR-RCSP-KN-BM-Prep for every instance. The comparison between LR-RCSP-KN-Prep and LR-RCSP-KN-BM-Prep shows no clear dominance: LR-RCSP-KN-Prep produces better \gls{lb}s in 3 instances, LR-RCSP-KN-BM-Prep in 3 instances, and both achieve equal \gls{lb}s in 33 instances. For \gls{ub}s, LR-RCSP-KN-BM-Prep and LR-RCSP-KN-HR-BM-Prep produce identical \gls{ub}s across all 39 instances, and both dominate LR-RCSP-KN-Prep and LR-RCSP-KN-HR-Prep. The LR-RCSP-KN-HR-BM-Prep variant achieves better or equal \gls{ub}s compared to LR-RCSP-KN-HR-Prep in all 39 instances, with strictly better \gls{ub}s in 36 instances and equal bounds in 3 instances. When comparing LR-RCSP-KN-Prep and LR-RCSP-KN-BM-Prep, the barrier method variant demonstrates clear superiority: LR-RCSP-KN-BM-Prep produces better \gls{ub}s in 36 instances and equal bounds in 3 instances, while LR-RCSP-KN-Prep never achieves a better \gls{ub}.

Comparing the best-performing configurations from each table, LR-RCSP-KN-HR-BM (without preprocessing) and LR-RCSP-KN-HR-BM-Prep (with preprocessing), across the 39 common instances reveals that preprocessing does not provide a clear advantage. For \gls{lb}s, the version without preprocessing achieves better \gls{lb}s in 10 instances, the version with preprocessing in 7 instances, and both achieve equal \gls{lb}s in 22 instances, indicating no dominance in either direction. For \gls{ub}s, both configurations produce identical \gls{ub}s across all 39 instances, demonstrating that preprocessing does not improve \gls{ub} quality. These results suggest that while preprocessing reduces instance sizes and may improve computational efficiency, it does not enhance the quality of the bounds obtained by the Lagrangean relaxation method.

\paragraph{Comparison between LR-RCSP-KN-HR-BM-Prep and Linear Relaxation}

To validate the quality of a Lagrangean Relaxation approach, we can compare it with the Linear Relaxation of the original problem. The Linear Relaxation is obtained by flexibilizing the integer variables of the original problem to be real-valued. The Linear Relaxation is a much easier problem to solve than the original problem, but it is a good approximation.

Table~\ref{tab:results-lagrange-vs-linear} presents a comparison between LR-RCSP-KN-HR-BM-Prep and the Linear Relaxation of Walk-CBRP-Prep. Regarding \gls{ub}s, LR-RCSP-KN-HR-BM-Prep achieves better \gls{ub}s in 36 instances, while both methods produce equal \gls{ub}s in 3 instances. These results demonstrate that the Lagrangean relaxation approach consistently outperforms the linear relaxation in terms of upper bound quality across the instance set. Considering the execution time, the linear relaxation is significantly faster than the Lagrangean relaxation, however the \gls{lr} do not exceed 90 seconds and scale better according to the instance size.
...
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.5]{images/deterministic-experiments/cd_csv_walk_lb.pdf}
	\caption{Iman-Davenport and Nemenyi tests for Lower Bounds: ID = 29,16; CV = 2,26; CD = 1,21} \label{fig:cd-walk-lb}
\end{figure}

\begin{table}[ht!]
	\centering
	\caption{Pairwise win counts for statistically equivalent CSV lower bounds approaches.} 
...
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.5]{images/deterministic-experiments/cd_csv_walk_ub.pdf}
