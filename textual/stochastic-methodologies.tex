...
This section presents the methodologies applied to solve the \gls{cbrp} in its stochastic version, called in this work as \gls{scbrp}. This section presents two approaches to apply stochasticity to the problem: the first one makes use of the simulation component presented in Section~\ref{chp:dengue-virus-spread-simulation} to generate scenarios for the stochastic model, and the second one use a simulation-optimization framework.

\section{Stochastic Optimization Models}\label{sec:cbrp-stochastic-models}

The \gls{scbrp}, computes, in the first-stage, a route to nebulize the blocks in the scenario $0$
